"""
backend/algorithms/zigzag/algorithm.py — ZigZag Algorithm (大少 2026-08-30 17:50 trigger)

凡人話: 1-to-1 port testing-page.js 嘅 ZigZag 算法去 Python backend,
對齊 frontend calculateZigZagFrontend, 拎 decision date / value / type 3 個新 field
畀 chart 上面嘅橙色旗仔 marker plot 喺決定嗰日。
...
class ZigZagAlgorithm(Algorithm):
    ...
    def run(self, klines: List[Dict[str, Any]], options: Dict[str, Any]) -> Verdict:
        ...
        3. return Verdict (meta 8 個 field 對齊 testing page 拎法 + production frontend pattern)

        大少 4.43.0 safety improvement #3: 拎走原本重複 logic (extract_hlc + auto_threshold_volatility
        + calculate_zigzag + build_extension_line + assign_sequence_numbers), 改 call run_zigzag helper
        ...
        #    (1 個 function 1 個 source of truth, 避免原本 2 個 function 重複維護 extract_hlc + auto_threshold_volatility + calculate_zigzag + build_extension_line + assign_sequence_numbers)
        ...
        return Verdict(
            ...
            meta={
                ...
                # - lastVerdict.meta.zigzagExtensionLine ← extension_line (向後兼容)
