...
- **Phase 4 done (2026-08-20)**:
  ...
  - 5 隻 stock verify (Spec Sync #39 大少 2026-09-07 00:02 trigger 重新跑, dataWindowDays=1260): HK.00700 騰訊 SIDEWAYS 0.90 (rules A,B,D,I,J, A+B special rule 補返後) / HK.00005 匯豐 UP 0.90 (rules A,I,J) / US.AAPL UP 0.90 (rules A,I,J) / US.MSFT UP 0.90 (rules A,I,J) / US.GOOGL SIDEWAYS 0.90 (rules A,B,C,D,I,J, A+B special rule 補返後). **之前 verify 結果有 3 隻 stock 對唔上**: HK.00700+US.GOOGL 因為 algorithm.py 冇 implement spec doc §5 line 109-111 嘅 A+B special rule (A+B 應該 SIDEWAYS 收斂三角但返 UP 0.9), US.AAPL 因為之前 5 隻 stock verify 嗰陣係手動估嘅 wishful thinking 唔係真實 algorithm output, fix algorithm 之後 US.AAPL 仍然 UP 0.9 對齊真實 algorithm output.
  - **v0.1.1 fix (2026-09-07 00:02, Spec Sync #39)**: 補 algorithm A+B special rule (commit `b259d1db`) + backend dataWindowDays 對齊 algorithm options (commit `febabd99`)
  - **v0.1.3 fix (2026-09-07 00:14, Spec Sync #40)**: H 真突破 guard (H fire + support_slope <= 0 → SIDEWAYS, 對齊 M2 self-check spirit, commit `7865544f`) + 3 個 self-check warning conditions (對齊 M2 self-check warning 永久 rule pattern). HK.01347 個 case fix 前返 UP 0.9 (錯, 應該 SIDEWAYS 對齊 M1+M2), fix 後返 SIDEWAYS 0.9. 5 隻 stock 重新跑 + HK.01347: HK.00700 warnings=2 / HK.00005 warnings=0 / US.AAPL warnings=1 (阻力 R²=0.584) / US.MSFT warnings=0 / US.GOOGL warnings=1 / HK.01347 warnings=1 (支撐 numPoints=3).
  - **v0.1.4 fix (2026-09-07 01:08, Spec Sync #41)**: Hurst+ADX gate 喺 Step 0.5 (對齊 Phase 1 (B3) 大少 trigger「如果先做B1+B3之後再加C2」). DFA (Detrended Fluctuation Analysis, 100 日) 計 Hurst 指數 (H>0.55 trending/H≈0.5 random walk/H<0.45 mean-reverting) + Wilder 14 日 standard 計 ADX (ADX>25 強/20-25 中/<20 弱). Gate 規則: H<0.45 OR ADX<20 → FAIL → return SIDEWAYS + 1 個 CONFLICT_STATE warning (system category), M7 自動降 M3 weight. Meta 加 `hurst` + `adx` field. 解決 audit 揭發嘅 M3 結構性問題 (404 隻 stock, Spec Sync #40 baseline: 一致率 28% / self-check 84% 觸發 / over-confident 46%). 6 隻 stock sample verify: HK.00700/HK.00005/US.AAPL/US.GOOGL/HK.01347 gate fail → SIDEWAYS 0.3 + 1w 對齊 M1+M2, US.MSFT gate pass (H=0.67, ADX=37.9) → UP 0.9. Frontend `modules/trendline.ts` v0.1.4 1:1 port 同步. 對應 commit `863bb22b` (fix).
...
- **Phase 7 done (2026-08-20)**:
  ...
  - Frontend `adapter.mjs` 拎走 M6 `analyzeVolatility` + helper, 換 fetch backend stub
...
| 2026-08-14 | **dataWindowDays 默認 100 → 1260 + 移除 CONFIG_DEFAULTS trigger** — testing page 默認值 100 (5 個月) 改做 1260 (5 年, 對齊 M9, 對 long-history 股票夠) + M1 v0.3.0 zmen + M9 移除 `CONFIG_DEFAULTS` warning trigger (因為 trigger 條件 `=== 100` 或 `=== 1260` 永遠會 trigger 變廢話) + 永久 rule: dataWindowDays 永遠用 5 年 (1260 日), 唔再用 100 日默認, 對 long-history 股票 verdict 更準, 短-history 股票 user 自行調小。 之後 M9 加 `auto-calibrate dataWindowDays` sub-task (9.8) 拎出嚟做 follow-up sprint | 大少 23:15 |
| 2026-08-14 | **Module Warning v1.1.0 — 2 Banner 分類 (Spec Sync #18)** — 大少 trigger「想分開兩個警告, 一個是系統/演算法/數據等會影響正常結果的警告, 另一個是對股票狀態的提醒但前提下所有結果都是無問題和準確的」 + 15 個 warning code 分 2 個 category (🔧 system 12 個 verdict 可能唔可信 / 📊 stock_state 3 個 verdict 已經準確) + `WARNING_CATEGORIES` + `CATEGORY_DISPLAY` template (2 種 impact/fix 統一) + `renderWarningBanners()` render 2 個獨立 banner + 28 個 `adapter.mjs` makeWarning 注入點統一跟 template (issue 保留各 module specific context) + 永久 rule: 2 banner 永遠獨立, impact/fix 永久跟 CATEGORY_DISPLAY template, 改 warning 注入點嗰陣 issue 必須保留 specific context + 4 份 spec doc sync | 大少 11:33 / 12:10 |
| 2026-09-06 | **M3 trendline chart overlay 修復 (Spec Sync #38)** — 大少 trigger「撳 M3 跑算法圖表裡看不到你說的那條線」, root cause 係 `algorithms/AS-03-cycle-detection/adapter.mjs` `renderTrendlineChartOverlay` line 3812 guard 拎 `verdict.meta.meta` 永遠 true → 永遠 early return, silent fail 因為 function 內 `console.warn + return` 唔 throw, testing page 嗰個 try/catch (line 1528-1534) catch 唔到, 大少肉眼睇唔到線但 console 冇 error. **fix 1** (commit `d663ef01`): 改 1 行 guard 拎 `verdict.meta` (唔再拎 `verdict.meta.meta`, Phase 4 拎走 frontend 改 fetch backend 之後 verdict shape 已經係 `verdict.meta.X`) + 修正 3 處 stale comment/console.warn. **feat 2** (commit `09ea4c21`): testing page chart-section 加 `#trendline-toggle-bar` (跟 MA toggle 永久 rule pattern), 入面 2 個 checkbox (🟢 支撐線 + 🔴 壓力線) + `lineSeries.applyOptions({ visible })` 即時切換 + `localStorage` 自動記住 (跟 8月19日 13:03 Config UX 模式永久 rule) + 出圖 sync (跟 4.66.7 ZigZag pattern). 永久 rule: `renderTrendlineChartOverlay` 嘅 guard 永遠拎 `verdict.meta`, frontend render function 拎 path 永遠 `verdict.meta.X`, 改 array/object access 之前必先 curl backend 拎 evidence 確認 (對齊 4.55.0 array evidence 永久 rule), chart overlay silent return testing page try/catch catch 唔到 → 撳跑算法之後必肉眼 verify chart overlay 有冇 render. 凡人話: 撳 M3 (AS-03-TL) 跑算法 → 圖表即見綠色支撐線 + 紅色壓力線, 撳 🟢/🔴 → 線即時消失 / 出現, reload page 保留 user 之前 set 過嘅 toggle state. 4 份 spec doc sync (AGENTS.md / ARCHITECTURE.md / PROJECT_SPEC.md / README.md) + cache bust `?v=2.3.146` → `?v=2.3.148` | 大少 16:47 |
| 2026-09-07 | **M3 A+B special rule 補返 + dataWindowDays backend 對齊 (Spec Sync #39)** — 大少 trigger「撳 M3 跑 00700 結果是上升加信心 90% 肯定有問題」, 揭發 `backend/algorithms/trendline/algorithm.py` `_derive_trendline_state` 從來冇 implement spec doc §5 line 109-111 嘅特殊規則「A + B 同時 fire (支撐升 + 壓力降) → 收斂三角形 = SIDEWAYS」, frontend 舊版 `backups/zigzag-frontend-2026-08-20/adapter.mjs` line 5386 都冇. **fix 1** (commit `b259d1db`): 加 1 行 `if "A" in ids and "B" in ids: return "SIDEWAYS"` 喺 H 真突破壓力之後, A 單獨 fire 之前. **fix 2** (commit `febabd99`): backend API `/api/algorithms/run` handler 將 `data_window_days` 同時放落 `options["dataWindowDays"]` 對齊 algorithm 入面 options.get("dataWindowDays") 拎法 (camelCase, 之前 options dict 冇呢個 key misleading code) + frontend M3 stub (`adapter.mjs` `analyzeTrendline` line 3466) 默認 100 改 1260 對齊 testing page 永久 rule 2026-08-14 23:15 (dataWindowDays 永遠用 5 年). 5 隻 stock 重新跑 verdict (大少 00:02 trigger): HK.00700 騰訊 UP 0.9 → SIDEWAYS 0.9 (rules A,B,D,I,J) / HK.00005 匯豐 UP 0.9 (唔受影響) / US.AAPL UP 0.9 (之前 README wishful 寫 SIDEWAYS, fix 後仍 UP 對齊真實 algorithm output) / US.MSFT UP 0.9 / US.GOOGL UP 0.9 → SIDEWAYS 0.9 (rules A,B,C,D,I,J). 凡人話: HK.00700 同 US.GOOGL 而家返 SIDEWAYS 對齊 README 期望, US.AAPL 改 UP 對齊真實 algorithm output. 4 份 spec doc sync (AGENTS.md / ARCHITECTURE.md / PROJECT_SPEC.md / README.md) + cache bust `?v=2.3.148` → `?v=2.3.149` | 大少 00:02 |
| 2026-09-07 | **M3 H 真突破 guard + self-check warning (Spec Sync #40)** — 大少 trigger「HK.01347 撳 M3 結果是上升這個有問題嗎」, 揭發 `_derive_trendline_state` H 真突破壓力 rule 冇考慮 long-term downtrend context. **fix** (commit `7865544f`): `_derive_trendline_state` 加 H 真突破 guard (H fire + support_slope <= 0 → SIDEWAYS, priority 第一), 避免 short-term breakout 蓋過 long-term downtrend context. HK.01347 個 case: support slope = -1.07, resistance slope = -2.29, 兩個都係 downtrend, 短線「真突破」H fire 蓋過 long-term context → over-confident UP 0.9, 但 M1 + M2 都係 SIDEWAYS, fix 後 verdict 變 SIDEWAYS 0.9 對齊 M1+M2. 加 3 個 self-check warning conditions (對齊 M2 self-check warning 永久 rule pattern): 支撐線太脆弱 (numPoints < 4 OR R² < 0.6) / 阻力線太脆弱 (numPoints < 4 OR R² < 0.6) / 通道太闊 (widthPct > 15%) → emit CONFLICT_STATE warning (system category) 落 verdict._warnings, frontend 透過 renderWarningBanners() 顯示 banner 提示大少「呢個 M3 verdict 唔可信」. 5 隻 stock + HK.01347 重新跑 evidence: HK.00700 warnings=2 (支撐+通道) / HK.00005 warnings=0 / US.AAPL warnings=1 (阻力 R²=0.584) / US.MSFT warnings=0 / US.GOOGL warnings=1 (支撐) / HK.01347 warnings=1 (支撐 numPoints=3). 凡人話: 3 隻 stock 之前 verdict 過於樂觀 (HK.01347 UP 而家 SIDEWAYS), 3 隻 stock 仍然 verdict 正確但加 warning 提示大少「呢個 M3 verdict 唔可信」. 4 份 spec doc + module spec doc MODULE-03-TRENDLINE.md §4.1 + §5 sync | 大少 00:14 |
| 2026-09-07 | **M3 Hurst+ADX gate (Spec Sync #41, Phase 1 (B3))** — 大少 trigger「如果先做B1＋B3之後再加你剛說的由『判斷者』變『證據提供者』」, 揀 Option 1 接受現狀 (大少 1:14 confirm「你的提議很好，Go」). 解決 Spec Sync #40 audit 揭發嘅 M3 結構性問題 (404 隻 stock baseline: 一致率 28% / self-check 84% 觸發 / over-confident 46%). **fix** (commit `863bb22b`): `backend/algorithms/trendline/algorithm.py` Step 0.5 新加 Hurst+ADX gate 喺 _derive_trendline_state 之前. DFA (Detrended Fluctuation Analysis, 100 日) 計 Hurst 指數 (H>0.55 trending/H≈0.5 random walk/H<0.45 mean-reverting) + Wilder 14 日 standard 計 ADX (ADX>25 強/20-25 中/<20 弱). Gate 規則: H<0.45 OR ADX<20 → FAIL → return SIDEWAYS + 1 個 CONFLICT_STATE warning (system category), M7 自動降 M3 weight. Meta 加 `hurst` + `adx` field (audit 對比用). 6 隻 stock sample verify: HK.00700/HK.00005/US.AAPL/US.GOOGL/HK.01347 gate fail → SIDEWAYS 0.3 + 1w 對齊 M1+M2, US.MSFT gate pass (H=0.67, ADX=37.9) → UP 0.9. Frontend `modules/trendline.ts` v0.1.4 1:1 port 同步 (computeHurst + computeAdx + detect() Step 0.5 gate). 4 份 spec doc + module spec doc MODULE-03-TRENDLINE.md §4.2 sync + AGENTS.md「M3 Hurst+ADX gate 永久 rule」section + 備份點 tag `restore-2026-09-07-m3-pre-b3-phase1` (commit `dbed0218`) + cache bust `?v=2.3.149` → `?v=2.3.150` / `ALGO_CACHE_BUST` `4.70.0` → `4.71.0` | 大少 01:08 |
| 2026-09-07 | **dataWindowDays frontend inputs 表單 audit + testing page 換 stock 強制 reset (Spec Sync #46)** — 大少 trigger「當輸入多次股票後就出了這問題, 怎樣可以根治」, 揭發 testing page 永久 rule 2026-08-14 23:15 (dataWindowDays 永遠 5 年) 1 個月嚟 frontend `adapter.mjs` 嘅各 module `inputs` 表單**漏咗改** (M3 / M4 / M5 / M6 4 個 module default 仍然 100, 漏 sync 永久 rule, M1 已經 1260, M2 維持 300 因為對齊 M2 spec 3 pairs alternating 結構), 加上 testing page 換 stock 嗰陣 `onAlgorithmChange()` 唔觸發, stale `dataWindowDays=100` 永遠累積; backend 對 100 日 K 線拎唔到嘅 stock (新股 / 細股 / 停牌 / FutuOpenD 拎唔到) 返 `ok=False` 400, frontend 冇 user-friendly 訊息, 大少睇到嘅模式就係「換 stock 撞中就壞」. **fix 3 層同步修** (1 個 Spec Sync commit): (1) **Frontend** `adapter.mjs` 6 處改: M3 inputs (line 3937-3943) default 100 → 1260 min 30 → 200 max 500 → 2520 + M4 indicators stub (line 3978) `|| 100` → `|| 1260` + M5 volume-price stub (line 2189) `|| 100` → `|| 1260` + M5 inputs (line 2583-2588) default 100 → 1260 min 80 → 200 max 500 → 2520 + M6 volatility stub (line 2643) `|| 100` → `|| 1260` + M6 inputs (line 2803) default 100 → 1260 min 80 → 200 max 500 → 2520; (2) **Frontend** `testing-page.js` 3 處改: `renderNumber` onChange handler clamp dataWindowDays 200-2520 對齊 backend Bulkowski condition + `runAlgorithm()` line 1330 開頭加 `currentOptions.dataWindowDays = 1260` 強制 reset + 同步落 DOM 避免 stale state 累積 + `runFullChain()` line 2385 開頭同樣強制 reset; (3) **Frontend** `adapter.mjs` `renderTrendlineResult` prepend user-friendly 黃色 box (「⚠️ 呢個 stock 拎唔到 K 線, 可能係新股 / 細股 / 停牌 / FutuOpenD 拎唔到, verdict 暫時 SIDEWAYS 0.3, 等有 K 線再跑」) 對 `INSUFFICIENT_DATA` warning case; (4) **Backend** `algorithm_runner.py` line 214 「冇 K 線」case 改返 `ok=True` + 帶 critical `INSUFFICIENT_DATA` warning (永久 rule §Module Warning v1.1.0 auto-enforce level, frontend WARNING_CATEGORIES 自動 derive system category 因為 verdict 唔可信冇 data); (5) **Backend** `trendline/algorithm.py` line 698-703 `n < min_required (30)` case 同樣改返 `ok=True` + 帶 `INSUFFICIENT_DATA` warning. 凡人話: 撳 M3 跑 99999 之前返 400 "Algorithm 內部錯誤 / HK.99999 1d 冇 K 線 data" → 而家返 200 + verdict SIDEWAYS 0.3 + 黃色 user-friendly box 提示大少「揀錯股票 / 新股 / 停牌」, verdict 仲可信 (永久 rule spirit). 永久 rule 加咗落 AGENTS.md「dataWindowDays frontend inputs 表單 audit 永久 rule (大少 2026-09-07 17:23 confirm)」section, 對齊 2026-08-14 23:15 + 2026-09-06 23:17 永久 rule pattern + 加新 checklist: testing page 任何 module 嘅 `inputs` 表單 `dataWindowDays` default 永遠 1260, min 200, max 2520, frontend `analyzeXxx` stub fallback `|| 1260`, renderNumber clamp 200-2520, 改 frontend 之後必 grep `dataWindowDays.*100` 確保冇漏網, testing page 換 stock 嗰陣強制 reset `currentOptions.dataWindowDays = 1260` 同步落 DOM, backend 「冇 K 線」/「n < 30」case 返 `ok=True` + INSUFFICIENT_DATA warning, frontend `renderTrendlineResult` prepend user-friendly box 對「INSUFFICIENT_DATA」case. 4 份 spec doc (AGENTS.md 永久 rule section + README.md 記錄) sync, module spec doc 對齊 (MODULE-03-TRENDLINE.md 唔變, MODULE-04 / 05 / 06 留俾將來 follow-up) + cache bust `?v=2.3.151` → `?v=2.3.152` / `ALGO_CACHE_BUST` `4.72.0` → `4.73.0`. **frontend additional bug 揭發** (Spec Sync #46 唔處理, 留俾將來 follow-up): testing-page.js line 1480 用 `verdict._warnings || []` 拎 warning banner, 但 backend `Verdict.warnings` 喺 top level (Verdict dataclass line 56), frontend 拎空 warning 唔 render, generic warning banner 路徑需要後續 fix; renderTrendlineResult 入面 user-friendly box 喺 renderResult 內部 prepend 避咗呢個問題 | 大少 17:23 |
