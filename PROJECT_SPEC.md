...
| 08 | **終極綜合判斷引擎** (Decision Engine — M8) | `modules/decision-engine.ts` + `backend/algorithms/decision_engine/` | **v2.0.0 (frontend) + v2.0.0 (Phase 10 backend Python, Spec Sync #30, 2026-08-20 22:08)** | ✅ **Sprint 2 收官 + Phase 10 backend port done (大少 22:08 揀方案 A 連做 Phase 9+10)**: 8 finalAction 決策樹 + Trading card + 9 scenarios + LLM hook + 5 adaptive params + L2 cache + 10 demo 股票 + 4 SVG chart + 重新校準 + 2.9 spec + **Phase 10 (Spec Sync #30)**: backend `decision_engine/algorithm.py` 24.3KB port frontend 9 step, 12 pytest pass, frontend `adapter.mjs` 拎走 `decisionEngineAdapter.analyze` 340 行 chain, 換 fetch backend stub, `algorithm_runner.py` M8 fallback 自動改用真 M8 (chain M9→M8 完整 work), 4 render function 拎 backend verdict shape. **AS-03 進度: 10/10 peer algorithm backend done** (M1+M2+M3+M4+M5+M6+M7+M8+M9+ZigZag). LLM hook 永久 rule (大少 13:30): `async generate_interpretation(ctx)` interface 預留, Sprint 2 用 hardcoded template, 將來 swap 落 LLM call. |
| **09** | **回測驗證** (Back Test — M9, 時光機驗證官) | `modules/back-test.ts` + `backend/algorithms/back_test/` | **v0.6.0 (frontend) + v0.6.0 (Phase 9 backend Python, Spec Sync #29, 2026-08-20 21:54)** | ✅ **Sprint 3 done + Phase 9 backend port done (大少 21:54 揀方案 A 連做 Phase 9+10)**: Replay engine + Coarse/Fine grid + Walk-Forward CV + Per-symbol optimal cache 30 日 + Forward return 永久 + Testing page entry 09 + HK.00700 pilot + M9 UI 升級 + **10 隻 (5 港 + 5 美) Pilot** + **Top 3 apply 落 M8** + **Backend 1w fix** + **Spec Sync #8 (2026-08-10 5 fix commits)** + **Spec Sync #29 (2026-08-20 21:54) Phase 9 backend port**: backend `back_test/algorithm.py` 37.5KB port frontend 5 個 sub-step (runReplay / runCoarseGrid / runFineTune / runAdaptiveWindow / runWalkForwardCV) + format helper, 11 pytest pass, frontend `adapter.mjs` 拎走 `backTestAdapter.analyze` 210 行 chain, 換 fetch backend stub (47 行), `algorithm_runner.py` 加 M7 Synthesizer verdict inject (chain M7→M9) + M8 Decision Engine decisionFn inject (Phase 10 done 之後自動, Phase 9 用 `_default_decision_fn` fallback). Algorithm ABC contract 對 M9 應用 (Phase 1 永久 rule), caller inject pattern 對 M9 extension (Phase 2 永久 rule, M9 拎 M7 verdict + M8 decisionFn 做 chain). **AS-03 進度: 9/9 peer algorithm backend done** (M1+M2+M3+M4+M5+M6+M7+M9+ZigZag, M8 仲未 port 留待 Phase 10). |
...
### Testing page config input 必須有 onChange handler 永久 rule (Spec Sync #31, 大少 2026-08-20 23:10)

**凡人話解釋**: testing page 任何 config input (number / checkbox / select / autocomplete) 必須有 onChange / onInput handler 連去 `currentOptions[key]` + 自動 re-render 對應 chart overlay, 唔可以等大少再撳「跑算法」先 update。
...
**套用**: 之後 M2 / M3 / M4 / M9 嘅 config input 全部跟呢個 pattern。

**Implementation**:
- `testing-page/testing-page.js` 抽 `refreshZigZagOverlay(code, period, threshold)` helper 封裝取 backend ZigZag + override `lastVerdict.meta` + 清舊 ZigZag/extension series + 通知 overlay 拎新 state + `renderChartOverlay` 重畫紫色線 + `renderDebugPanel` 重 update
- `runAlgorithm` L785-820 改用 `refreshZigZagOverlay` helper (33 行 inline code 變 3 行 call)
- 加 `#zigzag-threshold` input handler — `input` + `change` event listener, debounce 200ms, 撳即時 call `refreshZigZagOverlay`
...
**Implementation**:
- `testing-page/index.html` 改 layout: 3 個 element (`#run-status` / `#zigzag-controls` / `#zigzag-sequence-controls`) 由 `inputs section` 搬去 `chart-section` 入面, 排喺 `ma-toggle-bar` 之前, 加灰底圓角 (跟 ma-toggle-bar 一齊)
...
- **v0.1.5 fix (2026-09-07 17:23, Spec Sync #46)**: dataWindowDays frontend inputs 表單 audit + testing page 換 stock 強制 reset. testing page 永久 rule 2026-08-14 23:15 講明 `dataWindowDays` 永遠用 5 年 (1260), 但 frontend `adapter.mjs` 嘅 M3 / M4 / M5 / M6 4 個 module `inputs` 表單**漏咗改** (default 仍然 100), 加上 testing page 換 stock 嗰陣 `onAlgorithmChange()` 唔觸發, stale `dataWindowDays=100` 永遠累積, 撞到新股 / 細股 KlineCache 拎唔到 100 條 K 線就 400。3 層同步修: (1) `adapter.mjs` 6 處改 default 100 → 1260 min 80/30 → 200 max 500 → 2520, (2) `testing-page.js` `runAlgorithm()` + `runFullChain()` 撳跑前強制 reset `currentOptions.dataWindowDays = 1260` + 同步落 DOM, (3) backend `algorithm_runner.py` line 214 + `trendline/algorithm.py` line 698 改返 `ok=True` + 帶 `INSUFFICIENT_DATA` warning (永久 rule §Module Warning v1.1.0 auto-enforce level, frontend WARNING_CATEGORIES 自動 derive system category)。`renderTrendlineResult` prepend user-friendly 黃色 box 對「INSUFFICIENT_DATA」case 提示大少「揀錯股票 / 新股 / 停牌」。永久 rule 加咗落 AGENTS.md「dataWindowDays frontend inputs 表單 audit 永久 rule (大少 2026-09-07 17:23 confirm)」section, 對齊 2026-08-14 23:15 + 2026-09-06 23:17 永久 rule pattern. cache bust `?v=2.3.151` → `?v=2.3.152` / `ALGO_CACHE_BUST` `4.72.0` → `4.73.0`. M1 ma_alignment 已經係 1260, M2 hl_structure 維持 300 (對齊 M2 spec 3 pairs alternating 結構, Spec Sync #46 唔改).
