...
| 02 | HL Structure 高低點結構 | `modules/hl-structure.ts` + `backend/algorithms/hl_structure/` | v0.1.0 (frontend) + **v0.2.0 (Phase 9 backend Python, Spec Sync #37, 2026-09-06)** | ✅ | ✅ Production — **v0.2.0 backend 升級 (大少 2026-09-06 11:34 trigger)**: 19 步算法 (v0.1.0 18 步 + 2 步 v0.2.0 新加) + **Step 16 短線 mode** (60 日 window + adaptive window + weighted price, peak_trend + trough_trend 雙重 rising → override SIDEWAYS → uptrend) + **Step 17 突破 override** (above_peak + 量能 × 1.3 + [sub-trigger] consolidation_breakout 峰谷差距 < 5% → override SIDEWAYS → uptrend) + 4 個新 config option (`enableShortTermMode: True` / `shortTermWindowDays: 60` / `enableBreakoutOverride: True` / `enableConsolidationBreakout: True`) + meta 加 3 個 field (`short_term` / `breakout_override` / `version`) + 22 隻 M1 UP + M2 SIDEWAYS conflict stock 救得返 10 隻 (45%, 7 隻 B 突破 + 3 隻 A 短線 + 1 隻 consolidation_breakout 00019 太古). Trigger 設計跟 sub-scenario 永久 rule (大少 2026-08-16): 揀 3 隻 stock (00013 和黃 / 00019 太古 / 00506 中國食品) review confirm 落 code. 對應 commit Spec Sync #37. |
| 03 | Trendline 趨勢線法 | `modules/trendline.ts` + `backend/algorithms/trendline/` | v0.1.0 (frontend) + **v0.1.0 (Phase 4 backend Python, Spec Sync #23, 2026-08-20) + v0.1.1 chart overlay 修復 (2026-09-06 16:47, Spec Sync #38) + v0.1.2 A+B special rule 補返 (2026-09-07 00:02, Spec Sync #39) + v0.1.3 H 真突破 guard + self-check warning (2026-09-07 00:14, Spec Sync #40) + v0.1.4 Hurst+ADX gate (2026-09-07 01:08, Spec Sync #41)** | ✅ | ✅ Production — **v0.1.0 backend Python port** (大少 2026-08-20 20:50 trigger「搬M3加測試」): backend `trendline/algorithm.py` 27.3KB port frontend algorithm + 7 個 helper, 10 pytest pass, frontend `adapter.mjs` 拎走 506 行 `analyzeTrendline` + 7 個 helper, 換 fetch backend stub (34 行), 4 個 render function (`renderTrendlineResult` / `renderTrendlineRuleExplain` / `renderTrendlineChartOverlay` / `getTrendlineHelp`) 拎 `verdict.X` → `verdict.meta.X`, 5 隻 stock verify (大少 2026-09-07 00:14 trigger 重新跑, dataWindowDays=1260): HK.00700 騰訊 SIDEWAYS 0.90 (rules A,B,D,I,J, warnings=2) / HK.00005 匯豐 UP 0.90 (rules A,I,J, warnings=0) / US.AAPL UP 0.90 (rules A,I,J, warnings=1 resistance R²=0.584) / US.MSFT UP 0.90 (rules A,I,J, warnings=0) / US.GOOGL SIDEWAYS 0.90 (rules A,B,C,D,I,J, warnings=1). Algorithm ABC contract 對 M3 應用 (Phase 1 永久 rule), caller inject pattern 唔需要 (M3 同 M2 一樣 standalone, framework 保留), frontend render 拎 `verdict.meta.*` (Phase 2 永久 rule). **v0.1.1 chart overlay 修復 (大少 2026-09-06 16:47 trigger「撳 M3 跑算法圖表裡看不到你說的那條線」)**: `adapter.mjs` `renderTrendlineChartOverlay` line 3812 guard 拎 `verdict.meta.meta` 永遠 true → 永遠 early return, silent fail 因為 function 內 `console.warn + return` 唔 throw, testing page 嗰個 try/catch (line 1528-1534) catch 唔到, 撳 M3 (AS-03-TL) 跑算法之後圖表永遠冇綠色支撐線 + 紅色壓力線. Fix: guard 拎 `verdict.meta` (唔再拎 `verdict.meta.meta`, Phase 4 拎走 frontend 改 fetch backend 之後 verdict shape 已經係 `verdict.meta.X`) + 修正 3 處 stale comment/console.warn. 凡人話: root cause 確認 — curl backend `/api/algorithms/run?algo=trendline&symbol=HK.00700` 拎 evidence, `meta.supportLine` / `meta.resistanceLine` 直接喺 meta 下面, 冇 `meta.meta` wrapper. **M3 趨勢線 toggle 控制 (大少 2026-09-06 16:47 trigger)**: testing page chart-section 加 `#trendline-toggle-bar` (跟 MA toggle 永久 rule pattern), 入面 2 個 checkbox (🟢 支撐線 + 🔴 壓力線), `lineSeries.applyOptions({ visible })` 即時切換唔需要 re-call renderChartOverlay, `localStorage` 自動記住 user choice (跟 8月19日 13:03 Config UX 模式永久 rule), 出圖 sync (跟 4.66.7 ZigZag pattern). 永久 rule: M3 trendline toggle 跟 MA toggle 同樣 pattern (lineSeries.applyOptions + localStorage + 出圖 sync), 之後 M4/M5/M6 等加 chart overlay 嘅 module 都跟呢個 pattern. 對應 commit `d663ef01` (fix) + `09ea4c21` (feat). **v0.1.2 A+B special rule 補返 (大少 2026-09-07 00:02 trigger「撳 M3 跑 00700 結果是上升加信心 90% 肯定有問題」)**: `backend/algorithms/trendline/algorithm.py` `_derive_trendline_state` 補 spec doc §5 line 109-111 嘅特殊規則「A + B 同時 fire (支撐升 + 壓力降) → 收斂三角形 = SIDEWAYS」, 之前 algorithm.py 直接 `A in ids → return UP` 冇處理 special case (從來冇人 implement 落 code, frontend 舊版 backups/zigzag-frontend-2026-08-20/adapter.mjs line 5386 都冇), 影響 HK.00700 ['A','B','D','I','J'] 同 US.GOOGL ['A','B','C','D','I','J'] 返 UP 0.9 (錯, 應該 SIDEWAYS). 對應 commit `b259d1db` (fix) + `febabd99` (dataWindowDays backend 對齊 algorithm options + frontend M3 stub 默認 100 改 1260). **v0.1.3 H 真突破 guard + self-check warning system (大少 2026-09-07 00:14 trigger「HK.01347 撳 M3 結果是上升這個有問題嗎」)**: `_derive_trendline_state` 加 H 真突破 guard (H fire + support_slope <= 0 → SIDEWAYS, priority 第一), 避免 short-term breakout 蓋過 long-term downtrend context. HK.01347 個 case: support slope = -1.07, resistance slope = -2.29, 兩個都係 downtrend, 短線「真突破」H fire 蓋過 long-term context → over-confident UP 0.9, 但 M1 + M2 都係 SIDEWAYS, fix 後 verdict 變 SIDEWAYS 0.9 對齊 M1+M2. 加 3 個 self-check warning conditions (對齊 M2 self-check warning 永久 rule pattern): 支撐線太脆弱 (numPoints < 4 OR R² < 0.6) / 阻力線太脆弱 (numPoints < 4 OR R² < 0.6) / 通道太闊 (widthPct > 15%) → emit CONFLICT_STATE warning (system category) 落 verdict._warnings, frontend 透過 renderWarningBanners() 顯示 banner 提示大少. 5 隻 stock + HK.01347 重新跑 evidence: HK.00700 warnings=2 (支撐+通道), US.AAPL warnings=1 (阻力 R²=0.584), US.GOOGL warnings=1 (支撐), HK.01347 warnings=1 (支撐 numPoints=3), HK.00005/US.MSFT warnings=0. 對應 commit `7865544f` (fix). **v0.1.4 Hurst+ADX gate (大少 2026-09-07 01:08 trigger「如果先做B1＋B3之後再加你剛說的由『判斷者』變『證據提供者』」)**: `backend/algorithms/trendline/algorithm.py` Step 0.5 新加 Hurst+ADX gate 喺 _derive_trendline_state 之前. 用 DFA (Detrended Fluctuation Analysis, 100 日) 計 Hurst 指數 (量度 trending 持續性, H>0.55 trending/H≈0.5 random walk/H<0.45 mean-reverting) + Wilder 14 日 standard 計 ADX (量度趨勢強度, ADX>25 強/20-25 中/<20 弱). Gate 規則: H<0.45 OR ADX<20 → FAIL → return SIDEWAYS + 1 個 CONFLICT_STATE warning (system category), M7 自動降 M3 weight. Meta 加 `hurst` + `adx` field (audit 對比用). 解決 audit 揭發嘅 M3 結構性問題 (404 隻 stock, Spec Sync #40 baseline): 一致率 28% (M3 同 M1+M2 對唔足) / self-check 84% 觸發 (M3 結構脆弱) / over-confident 46% (信心過高但 verdict 唔對). 6 隻 stock sample verify: HK.00700/HK.00005/US.AAPL/US.GOOGL/HK.01347 gate fail (H<0.45 OR ADX<20) → SIDEWAYS 0.3 + 1w 對齊 M1+M2, US.MSFT gate pass (H=0.67, ADX=37.9) → UP 0.9. Frontend `modules/trendline.ts` v0.1.4 1:1 port 同步 (computeHurst + computeAdx + detect() Step 0.5 gate). 對應 commit `863bb22b` (fix). |
...
| 2. HL Structure | `MODULE-02-HL-STRUCTURE.md` | v0.2.0 (Peak-Trough + 短線 mode 60 日 + 突破 override + consolidation_breakout) |
...
### 15.45 純 branch 還原點永久 rule (大少 2026-09-08 21:00 trigger, 大項目之前必做)

### 大少 trigger
9月8日 21:00「以後的還原點是用分支來做, 名字前要有 \"Backup-\", 和現在的分支還原點一樣」— 拎走 Sscript pattern (annotated tag + backup branch + restore script) + Backup Admin Page (§15.45 + §15.53 + §15.54 拎走), 改為**純 branch 還原點 pattern**。

### 凡人話解釋
之前 §15.45 嘅 Sscript pattern (annotated tag + backup branch + restore script) 配 §15.54 Backup Admin Page 統一管理所有還原點, 太複雜大少用唔著個 page, 9月8日 21:00 拎走成個 stack, 改為**純 branch 還原點 pattern**: 大少只係開 1 個 `Backup-YYYY-MM-DD-description` branch, push 去 origin, 之後做改動, 拎返就 `git checkout Backup-xxx` 或者 merge 返 main。
...
### 15.53 ZigZag 4.53.0 Sscript 還原點永久 rule (大少 2026-08-31 11:59 trigger) — 已拎走 (大少 2026-09-08 21:00 trigger)

**狀態**: 拎走 (大少 9月8日 21:00 trigger「以後的還原點是用分支來做, 名字前要有 Backup-」)。

原本 §15.53 ZigZag 4.53.0 Sscript 還原點永久 rule (annotated tag + backup branch + restore script) 拎走, 改為「純 branch 還原點 pattern」, 詳見上方「15.45 純 branch 還原點永久 rule」段。對應 `f4adfe05` commit (Sscript commit) 拎走。

### 15.54 Backup Admin Page 永久 rule (大少 2026-08-31 12:00 trigger) — 已拎走 (大少 2026-09-08 21:00 trigger)

**狀態**: 拎走 (大少 9月8日 21:00 trigger「以後的還原點是用分支來做, 名字前要有 Backup-」)。

原本 §15.54 Backup Admin Page 永久 rule (`backup-admin/` + `backend/api/backup_admin.py` + 5 個 endpoint) 拎走, 改為「純 branch 還原點 pattern」, 詳見上方「15.45 純 branch 還原點永久 rule」段。對應 `chore: 加 Backup Admin Page` commit 拎走。
...
- ✅ B. Sscript set helper: Frontend「+ 設定新還原點」掣 → modal 輸入 name + reason → Backend `POST /api/backup-points/set` 自動 generate script + tag + branch + push (對齊「純 branch 還原點永久 rule」永久 rule)
...
### 教訓
- 大少 trigger「保留 tag」+「可能會再用」= 對齊「純 branch 還原點永久 rule」永久 rule + 12:08 user memory 永久 rule
