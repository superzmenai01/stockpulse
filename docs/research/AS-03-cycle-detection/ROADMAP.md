...
| 09 | **Module 9: Back Test (M9 時光機驗證官)** | ✅ **Sprint 3 done (大少 2026-08-08 23:55, 6 commits: 9.1-9.6) + Spec Sync #8 done (2026-08-10, 5 fix commits)** | M9 spec doc: `MODULE-09-BACK-TEST.md` v0.6.0. Replay engine (逐日 replay M8 verdict + 5/10/20 日 forward return) + Coarse grid (3×3=9 params combinations) + Fine tune top 5 ±20% + Adaptive window 6→9→12→15→18 個月 (samples < 30 自動 extend) + **Walk-Forward CV** (Spec Sync #8: `numFolds 3 → 1` + `tuneRatio 0.67 → 0.6` 過 HLStructure 99 bar gate, 1 fold tune 756 + validate 504) + Per-symbol optimal params (30 日 expiry) + Forward return record (永久保留, 6 月半衰期 weighted stats) + **Testing page entry 09 — AS-03-BT** (Spec Sync #8: dataWindowDays default 252 → 1260, UI label 動態化) + **UI 升級 (9.7)** 3 SVG + 6 色標 + 永遠 full show 過往判決 + 大少話你知 box + 2 button + HK.00700 pilot (3 folds ✅, 24ms 跑完). **Spec Sync #8 5 fix commits**: M9 POST silent fail + UI error banner (`788ccab7`) + debug log 移 (`ea75ebd1`) + Trade Journal 編號 M10→J (`ffaa7593`) + walk-forward tune gate (`01aed775`) + UX 升級 (`6bd4e2d3`). 776 assertions pass. |
...
| 9 | Back Test (M9, 9.1-9.6 done + Spec Sync #8 5 fix commits 2026-08-10) | ✅ Sprint 3 done + Spec Sync #8 done | 2026-08-08 (Sprint 3) + 2026-08-10 (Spec Sync #8) | v0.6.0 | — | [MODULE-09-BACK-TEST.md](./MODULE-09-BACK-TEST.md) |
