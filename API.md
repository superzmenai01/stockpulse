...
**Phase 8 — synthesizer (M7)**: 終極綜合判定 (跟 `modules/synthesizer.ts` 1:1 port, Plan A 拆返 M7+M8, 5 個 sub-step: SSI 戰略強度 + TCM 戰術交叉驗證 + Alignment + Grade 8 個評級 + Kelly 倉位), frontend 拎 verdict 之後 render 拎 `verdict.meta.*` 拎 backend 兼容 shape (state / cycleLabel / confidence / ssi_score / ssi_breakdown / tcm_matrix / alignment_score / grade / grade_score / kelly_fraction / kelly_numeric / module_summary), caller inject pattern: `algorithm_runner` 自動拎 6 個 module standard verdict interface (state / confidence / base_weight / max_drawdown_estimate / rules_fired) inject 落 options, Synthesizer 唔可以直接拎 K 線拎 M1-M6。

**Phase 9 — back_test (M9, Spec Sync #29, 2026-08-20 21:54)**: 回測驗證 — 時光機驗證官 (跟 `modules/back-test.ts` v0.6.0 1:1 port, Sprint 3 收官 9.1-9.7 全部 done), 5 個 sub-step: `runReplay` (用歷史 K 線 replay 之前嘅判決, 對比 5/10/20 日後真實升跌, 揾 hit rate) + `runCoarseGrid` (9 candidates kelly × rsi × ssi) + `runFineTune` (top 5 ±20% 微調 = 30 candidates) + `runAdaptiveWindow` (6→18 個月, samples < 30 自動 +3 個月) + `runWalkForwardCV` (3 folds rolling, 揾 out-of-sample 真實表現最高嘅 fold, 計 avg validate score + stability = 1 - stddev/mean)。Frontend 拎 verdict 之後 render 拎 `verdict.meta.*` 拎 backend 兼容 shape (state / cycleLabel / confidence / walkForwardResult / folds / overall / bestParams / avgValidateScore / stabilityScore / totalValidateSamples / foldsCount / postErrors / m7Context)。Caller inject pattern 對 M9 extension: `algorithm_runner` 自動跑 M7 Synthesizer 拎 verdict (chain M7→M9) + M8 Decision Engine decisionFn inject (Phase 10 done 之後自動拎真 M8, Phase 9 fallback 拎 `_default_decision_fn` 返 SIDEWAYS)。跑完 walk-forward CV 之後, M9 自動 POST optimal 落 `/api/adaptive-params/{symbol}/back-test` cache (30 日 expiry, in-process call 落 `services.adaptive_params_cache.save_optimal`, 唔走 HTTP) + 逐條 forward return records 落 `/api/adaptive-params/{symbol}/forward-return` cache (永久保留, 半衰期 180 日 weighted stats)。

**Phase 10 — decision_engine (M8, Spec Sync #30, 2026-08-20 22:08)**: 終極綜合判斷引擎 (跟 `modules/decision-engine.ts` v2.0.0 1:1 port, Sprint 2 收官 2.1-2.9 全部 done), 9 個 step: 數據驗證 + majority state (6 個 modules state 最大 group) + weighted avg max_drawdown_estimate (用 base_weight 加權) + raw RSI (從 indicators verdict) + applyAdaptiveParams 落 Synthesizer (M9 optimal params override) + 8 finalAction 決策樹 (priority chain: TRAP > TRANSITION > SELL > REDUCE > WAIT > HOLD > ADD > BUY) + Trading card 4 fields (3 個 volatility bucket: high vol ±2.5% / mid vol ±1.5% / low vol ±1.0%) + 短期走勢 9 scenarios (3 scenarios × 3 timeframes: 5/10/20 日, 概率 25/50/25) + LLM hook interpretation (8 個 finalAction 各自白話 + 揸車比喻)。Frontend 拎 verdict 之後 render 拎 `verdict.meta.*` 拎 backend 兼容 shape (state / cycleLabel / confidence / final_action / final_action_reason / trading_card / short_term_forecast / interpretation / module_verdicts / synthesizer_verdict / optimal_data / max_drawdown_estimate / evidence)。Caller inject pattern 對 M8 extension: M8 verdict 永久 embed M9 optimal_data (AS-03 chain rule M9→M8, 撳 M8 即刻見到 M9 拎咩 optimal 設定), M8 verdict 拎 M7 Synthesizer verdict + 6 個 module standard verdict + M9 optimal params (chain M7→M9→M8 完整 work)。LLM hook 永久 rule (大少 2026-08-08 13:30): `async generate_interpretation(ctx)` interface 預留, Sprint 2 用 hardcoded template, 將來 swap 落 LLM call (OpenAI/MiniMax/Kimi 任何), 唔使改 decide() call site。**AS-03 進度: 10/10 peer algorithm backend done** 🎉 (M1+M2+M3+M4+M5+M6+M7+M8+M9+ZigZag 全部 backend port 完成)。
