...
const ALGO_CACHE_BUST = '4.93.0';  // 大少 2026-09-10 23:45 — frontend 8 個 entry point normalize 拎 backend emit `verdict.warnings` 落 `_warnings` (8月31日 Batch 2 永久 rule 512a2138, backend emit 已經永久改 `warnings` 冇 underscore 但 frontend 仲拎 `_warnings` 拎唔到 → 強跌/弱勢/Hurst+ADX gate 等 stock_state banner 永遠唔出); 6 個 fetch backend function (M1/M3/M4/M5/M6 新加 + M2 Fix A 拎 path 改返) + decisionEngineToStandardVerdict (M7 aggregator) + testing page 入口 banner normalize 全部對齊 backend emit shape; 大少 9月10日 23:42 trigger「00700 很明顯這個表有很大問題」撳跑 M1 強跌睇唔到 stock_state banner 揭發; 對齊 §Cache bust self-check 永久 rule 21:24 sync bump ?v=2.3.171 → 2.3.172; 對齊 §Module Warning v1.0.0 + v1.1.0 + §M7 v2.0.1 Fix D spirit「frontend display 永遠對齊 backend verdict shape」
