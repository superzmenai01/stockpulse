...
const ALGO_CACHE_BUST = '4.94.0';  // 大少 2026-09-10 23:50 — frontend `lib/warnings.mjs` 2 處 categorize 邏輯拎 backend emit `warning.category` 優先, 對齊 backend verdict shape (大少 23:48 hard reload 撳跑 M3 00700 揭發 frontend 顯示 1 個 system + 3 個 stock_state 但 backend emit 7 個 warning 全部 category="system", frontend WARNING_CATEGORIES dict 重新 derive 拎 stock_state 錯晒); 對齊 §Cache bust self-check 永久 rule 21:24 sync bump ?v=2.3.172 → 2.3.173; 對齊 §Module Warning v1.1.0 (backend emit warning 永遠有 category field) + §M7 v2.0.1 Fix D spirit (frontend display 永遠對齊 backend verdict shape)
