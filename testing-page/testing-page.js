...
const ALGO_CACHE_BUST = '4.95.0';  // 大少 2026-09-11 11:47 — frontend `lib/warnings.mjs` 3 處拎 backend emit `warning.issue` / `warning.impact` / `warning.fix` 優先, 對齊 backend verdict shape (大少 11:45 hard reload 撳跑 M3 00700 揭發 frontend 顯示「影響: ? 修復: ?」拎唔到 backend emit 嘅 impact / fix field, frontend 拎 `warning.debug.impact` / `warning.debug.fix` 拎唔到, 因為 backend 用 top-level shape, frontend 內部 `makeWarning` helper 用 `debug.{}` 包住); 對齊 §Cache bust self-check 永久 rule 21:24 sync bump ?v=2.3.173 → 2.3.174; 對齊 §Module Warning v1.1.0 + §M7 v2.0.1 Fix D spirit + 前 commit 74d52405 (category 拎 backend emit 優先 pattern)
