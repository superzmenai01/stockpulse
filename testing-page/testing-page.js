...
async function runAlgorithm() {
  ...
  try {
    ...
    // 凡人話: 撳跑 M1 嗰陣, chart 上面 init chart banner element (activeCycle='all' 預設 hidden, 因為 Tab A 默認)
    ...
    if (brackChartBannerEl) {
      if (currentAdapter.id === 'AS-03-MA') {
        try {
          ...
          brackChartBannerEl.innerHTML = renderBrackTestChartBanner(verdict, 'all');
          lastBrackChartBanner = { panel: brackChartBannerEl, verdict, activeCycle: 'all' };
        } catch (err) {
