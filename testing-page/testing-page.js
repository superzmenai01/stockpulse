...
//   之前 4.66.0 commit 寫嘅「localStorage 自動記住大少 choice」係 Mavis 推斷, 大少真正意思係永遠 default Off
//   撳 toggle 即時 render marker (8月19日 13:03 Config UX 模式即時 re-render 嗰個 spirit 保留), 但 reload 永遠返 unchecked
//   拎走 LS_KEY_SHOW_MARKERS + getShowMarkers + setShowMarkers (冇用,拎走清 clean code)
//   對齊 4.66.0 原始 trigger「做返一個開關制是控制這個P點和觸發點的 預設是關的」, 拎走 user choice 自動記住
function getShowMarkersDefault() {
  return false;  // 4.66.5 永久 rule: 永遠 default false
}
 ...
 */
// 大少 2026-09-15 22:27 trigger — Brack Test 指定日期範圍完整重跑 (v0.4.3) 修 ZigZag 沒有重跑 issue
// 凡人話: 大少 trigger「在指定日期內跑 Brack Test 但發現 zigzag 和 P 點 沒有重跑」
// Root cause: `fetchBackendZigZag` 之前 hardcode `data_window_days: '1260'` (5 年) + 冇 add `date_from`/`date_to` query params, 所以撳「執行」button 之後 chart reset (filtered, 11 條 K 線), 但係 ZigZag verdict 仍然拎 5 年嘅 points, ZigZag 紫線 + P 點 marker 嘅 time 唔喺 chart visible range 內
// Fix: 加 dateFrom + dateTo + dataWindowDays optional args, fetch URL add date_from/date_to params + 用 caller value (對齊 K-line Cache 永久 rule spirit 8月22日 23:20「Frontend 拎 data, Backend 拎 K 線」)
async function fetchBackendZigZag(code, period, thresholdMode, manualThreshold, lookback, multiplier, signal, dateFrom, dateTo, dataWindowDays) {
  // 大少 22:27 trigger — data_window_days 用 caller 傳入嘅 filtered K 線 length, 唔再 hardcode 1260
  // 凡人話: 撳「執行」button 嗰陣 dataWindowDays = klines.length (filtered), default 1260 (5 年, 對齊既有 4.17.0 behavior)
  const effectiveDataWindowDays = dataWindowDays || 1260;
  const params = new URLSearchParams({
    algo: 'zigzag',
    symbol: code,
    period: period,
    data_window_days: String(effectiveDataWindowDays),
  ...
  // 大少 22:27 trigger — 加 date_from / date_to query params (對齊 backend api/kline.py line 47 + api/algorithms.py 既有 start/end pattern)
  // 凡人話: 大少撳「執行」button 帶 date_from/date_to, backend ZigZag 拎 filtered K 線 → emit ZigZag verdict 對齊 filtered K 線
...
// 大少 2026-09-15 22:27 trigger — Brack Test 指定日期範圍完整重跑 (v0.4.3) 修 ZigZag 沒有重跑 issue
// 凡人話: 大少 trigger「在指定日期內跑 Brack Test 但發現 zigzag 和 P 點 沒有重跑」, 加 dateFrom / dateTo / dataWindowDays optional args 拎 caller 嘅 filtered K 線 range + length
async function fetchAndInjectBackendZigZag(thresholdMode, manualThreshold, lookback, multiplier, dateFrom, dateTo, dataWindowDays) {
  ...
  console.log(
    ...
    `dateFrom=${dateFrom || '❌None'}, dateTo=${dateTo || '❌None'}, dataWindowDays=${dataWindowDays || 'default 1260'}`
  );
  ...
  try {
    console.log(`[ZigZag Debug] fetch start: ${BACKEND_URL}/api/algorithms/run?algo=zigzag&symbol=${code}&period=${period}&data_window_days=${dataWindowDays || 1260}&threshold_mode=${thresholdMode}&lookback=${lookback}&multiplier=${multiplier}${dateFrom ? `&start=${dateFrom}` : ''}${dateTo ? `&end=${dateTo}` : ''}`);
    // 大少 22:27 trigger — 傳落 dateFrom/dateTo/dataWindowArgs 落 fetchBackendZigZag 對齊 filtered K 線 range
    const verdict = await fetchBackendZigZag(
      code, period, thresholdMode, manualThreshold, lookback, multiplier, signal, dateFrom, dateTo, dataWindowDays
    );
...
//
// 大少 8月31日 17:42 trigger (修改版 20:51 + 20:57) — M1 console log 加 Threshold % + 觸發點 (Trigger 確認點): ALGO_CACHE_BUST = '4.57.0'
...
//     ✅ C3: 拎走 adapter.mjs P 點 arrow + 紅色觸發點 circle marker 整個 block (~66 行 marker build + setMarkers)
...
//     ✅ 凡人話: 大少 trigger「之前做的 Point, 旗仔, 觸發點等等, 只保留 zigzag 的連線, 其他都不要」, 拎走晒 frontend 5 個 non-line visual elements (紫色 P 點 arrow / 紅色觸發點 circle / 鮮綠色 close extension line / 鮮綠色 1 號 marker / 橙色 #FF9800 旗仔), chart 只 render 紫色 ZigZag 折線
//     ✅ 對齊 8月29日 22:44 永久 rule「所有改動要 confirm」:大少明確 trigger「拎走 P 點 / 旗仔 / 觸發點 / 鮮綠線」先做
//   對應 commit: refactor(frontend): 拎走 ZigZag non-line visual elements (P 點 / 旗仔 / 觸發點 / 鮮綠線), chart 只 render 紫色折線
...
//   ✅ 唔拎返 4.53.0/4.61.5 拎走嘅其他嘢: 橙旗 (4.42.2) / 鮮綠 close extension 線 (4.8.3) / 紅色觸發點 (4.61.5) / P 點 toggle 同 spinbutton (4.53.0)
...
// 大少 2026-09-02 00:52 trigger (Fix 4.66.0) — 拎返 P 點 + 鮮紫觸發點 marker toggle (4.53.0 拎走嘅 #show-sequence 拎返, 預設關): ALGO_CACHE_BUST = '4.66.0'
...
//   ✅ 對齊 4.65.0 永久 rule: P 點 + 鮮紫 trigger 一齊 toggle (大少 trigger「控制這個P點和觸發點的」)
...
//   ✅ 4.66.5 fix (大少 9月2日 07:34 trigger「把紅框這個制預備是 Off 的」): 拎走 4.66.0 嗰個 localStorage 自動記住嘅 spec
//   ✅ 拎走 LS_KEY_SHOW_MARKERS + getShowMarkers + setShowMarkers, 改為永遠 default false (line 95-105 + 1746-1747 + 1756)
//   ✅ 撳 toggle 仍然即時 render marker (8月19日 13:03 Config UX 模式即時 re-render 嗰個 spirit 保留)
...
// 大少 2026-09-07 21:53 — M2 frontend warning UI 修復: ALGO_CACHE_BUST = '4.73.0' → '4.74.0' (adapter.mjs 3 處改: analyzeHLStructure propagate _warnings 落 verdict 頂層 (Fix A, 對齊 §Module Warning v1.0.0) + decisionEngineToStandardVerdict propagate _warnings 落 standard verdict (Fix B, 對齊 §M2 self-check weight 折扣 永久 rule, M7 algorithm.py line 410 拎 v.warnings 觸發 discount 0.15→0.05) + renderHLStructureResult 加 renderWarningCards 喺 verdict card 內 (Fix C, 對齊 §Module Warning v1.1.0 2 banner + 個別 card WarningCard); 凡人話: 撳 M2 跑 00700 而家見到 🟡 CONFLICT_STATE warning banner + 個別 card WarningCard, M7 綜合判斷嗰陣自動將 M2 weight 由 15% 折到 5%, 其他 5 個 module 等比例補返 10%; 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.152 → ?v=2.3.153; commit fix 即將 push (Spec Sync #47))
// 大少 2026-09-08 23:30 — M3 audit 修 Bug 1+2: ALGO_CACHE_BUST = '4.74.0' → '4.75.0' (adapter.mjs 18 處改: 拎走全部 `verdict.meta.meta?.X` → `verdict.meta.X` (M3 renderTrendlineResult 3 處 + M4 renderIndicatorsResult 15 處, 對齊 2026-09-06 16:47 §M3 trendline chart overlay 修復永久 rule spirit, 之前 fix 漏咗), backend 2 處改: algorithm.py 拎走 inner-scope `from backend.services.warning_collector import make_warning` 改做 module level import 對齊 synthesizer/algorithm.py:38 pattern (修 100% runtime fail 嘅 UnboundLocalError), 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.153 → ?v=2.3.154)
// 大少 2026-09-09 07:20 — M4 frontend sync 修 ReferenceError: ALGO_CACHE_BUST = '4.76.0' → '4.77.0' (adapter.mjs renderDetailedExplanationIndicators 開頭加 6 條 local variable 拎 verdict.meta.hurst/adx/regimeGate/m1State/selfCheckTriggered/originalConfidence, 之前大少 9月9日 07:20 撳 M4 跑 testing page 觸發 ReferenceError: hurst is not defined 喺 line 4188:57, 對齊 §M3 trendline chart overlay 修復永久 rule spirit「改 array/object access 之前必先 curl backend 拎 evidence 確認」, 凡人話: renderDetailedExplanationIndicators 係獨立 function, 唔可以攞 renderIndicatorsResult 嘅 local var, 一定要喺呢度重新拎 verdict.meta.*, 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.155 → ?v=2.3.156)
// 大少 2026-09-09 07:23 — M4 chart overlay 修 silent fail: ALGO_CACHE_BUST = '4.77.0' → '4.78.0' (adapter.mjs renderIndicatorsChartOverlay line 4282 guard 拎 verdict.meta.meta 永遠 true → 早 return, RSI + MACD line series 永遠唔 render 落 chart, console warning 但 try/catch catch 唔到; 對齊 §M3 trendline chart overlay 修復永久 rule spirit「Frontend render function 拎 path 永遠 verdict.meta.X, 唔好再寫 verdict.meta.meta.X」, 凡人話: 之前 commit d663ef01 拎走 18 處 verdict.meta.meta?.X → verdict.meta.X 但漏修呢個 guard, 大少 07:23 trigger console warning 之後修; backend evidence: curl /api/algorithms/run?algo=indicators&symbol=US.MSFT 拎 rsiSeries 315 條 + macdSeries 329 條, 改完 reg gate pass 嗰陣 RSI + MACD line series 會 render 落 chart; 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.156 → ?v=2.3.157)
// 大少 2026-09-09 07:27 — M4 chart overlay 修 shape consistency: ALGO_CACHE_BUST = '4.78.0' → '4.79.0' (backend algorithms/indicators/algorithm.py reg gate fail 早 return 嗰陣 emit rsiSeries: [] + macdSeries: [] 落 meta dict 對齊 verdict shape 一致性, frontend renderIndicatorsChartOverlay guard 拎 rsiSeries/macdSeries.length === 0 雙重保護兜底, 對齊 §M3 trendline chart overlay 修復永久 rule spirit「Frontend render function 拎 path 永遠 verdict.meta.X」+ 凡人話 backend 改 shape frontend 對齊 shape pattern; backend evidence: curl /api/algorithms/run?algo=indicators&symbol=HK.00700 拎 rsiSeries=[] + macdSeries=[] (empty list, 之前係 undefined 觸發 warning); restart backend (./start.sh, 對齊 8月31日 11:01 hot-reload 永久 rule); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.157 → ?v=2.3.158)
// 大少 2026-09-09 07:33 — M4 chart overlay 修 false positive warning: ALGO_CACHE_BUST = '4.79.0' → '4.80.0' (adapter.mjs renderIndicatorsChartOverlay line 4294 guard 改分 2 個 case, 對齊 §M4 v0.2.0 backend commit 9月9日 07:27 shape consistency intent: Case 1 backend K 線唔夠 (INSUFFICIENT_DATA) 早 return path, 完全冇 emit rsiSeries/macdSeries field → 觸發 warning 提示大少 (silent fail, 真係 missing); Case 2 backend reg gate fail (H<0.45 OR ADX<20) 早 return path, emit empty array → 凡人話正常 (冇 series 數據), frontend 唔 render line series 0 條, 直接 return 唔 trigger warning. 之前 v0.2.0 寫 `!rsiSeries || rsiSeries.length === 0` 拎到 [] 都 trigger, 違反 backend commit spirit, 凡人話 false positive 嘈住大少; backend evidence: curl /api/algorithms/run?algo=indicators&symbol=HK.00700 拎 state=SIDEWAYS conf=0.3 + rsiSeries=[] + macdSeries=[] + 1 個 CONFLICT_STATE info warning, 凡人話 verdict 已經準確; frontend-only fix, 唔需要 restart backend; 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.158 → ?v=2.3.159)
...
// 大少 2026-09-14 23:38 — M1 Brack Test chart marker 加強 visibility fix (大少 23:38 trigger「你的Marker 是用了什麼顏色？我真的找不到, 有沒有辦法可以肯定你真的加了上去」): Root cause 確認 (testing-page.js line 1797 chart background 是白色 #ffffff, 唔係黑底): marker 用 BRACK_TEST_CYCLE_COLOR_MAP 對應色 (e.g. downtrend_bounce=#F5B7B1 淡紅, weak_downtrend=#F1948A 淺紅, uptrend_correction=#A8D5BA 淡綠), 對比白色 background 對比度低, 淡色 marker 溶入白底; size 1 (LWC v5 預設細) 對比 14px K 線太細. Fix 3 個 modify: (a) marker size 1 → 2 (LWC v5 medium, 對齊 plan 不破壞 design); (b) marker 加 borderColor:'#000' + borderWidth:1 (黑色 outline 對比白色 chart background, 淡色 marker 對比度提升); (c) marker text 顯示 cycle 中文 label + displayIndex (e.g. 「下跌反彈 #42」, 大少肉眼 scan 易搵到). 同時加 console.log 證實 markers 真係有 add 落 chart handle (markers.length + first marker detail + handle.markers.length 對比, 對齊 §M3 trendline chart overlay 修復永久 rule「testing page chart overlay 視覺 verify」spirit). 對齊 §M3 trendline chart overlay 修復永久 rule (9月6日 16:47) — render function 永遠拎 verdict.points. 凡人話 verify (對齊 9月6日 16:47 永久 rule): 大少 hard reload testing page + 撳跑 M1 (HK.00700) + Tab B 揀「下跌反彈中」, 肉眼 verify (a) banner 個 dot 用淡紅 fill + 白色 border (b) K 線圖入面 20 條 (半年內) 淡紅 circle marker + 黑色 outline + size 2 + text 「下跌反彈 #42」 etc 同步 render, console.log 印出 markers=20 + sample[0].color=#F5B7B1 + sample[0].size=2 + handle.markers.length=20; 對齊 §M1 sub-scenario 永久 rule「改任何 sub_scenario display 都要即刻 update spec doc」spec doc §7 Chart top banner 更新 + Change log v0.1.0-fix9 entry; 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.189 → ?v=2.3.190 + 對齊 §Backend hot-reload 永久 rule (8月31日 11:01) 改 backend algo 必 restart + curl verify (backend 唔改所以唔需要); 凡人話 (8月14日 19:02 trigger) + 「用『取』唔用『拎』」(8月20日 trigger)
// 大少 2026-09-14 23:43 — M1 Brack Test chart marker 唔見嘅真正 root cause fix (大少 23:43 trigger「你自己看看真的沒有Marker」+ screenshot evidence: chart 入面冇任何 marker, console log `lastChartRefs 或 lastKlines 缺失, skip chart overlay`): Root cause 確認: testing-page.js 嘅 `lastChartRefs` (line 783 top-level let) + `lastKlines` (line 784 top-level let) 係 module-level 變量, 唔會自動 attach 落 `window` global object. 但 adapter.mjs `_brackTestRunHandler` (line 6079) + `_ModeHandler` (line 6063) + `_CycleHandler` (line 6084) 都拎 `window.lastChartRefs` 同 `window.lastKlines`, 所以呢 2 個 reference 永遠 `undefined` → `renderBrackTestChartOverlay` 嘅 `if (chartRefs && klines && klines.length)` 永遠 `false` → 早期 return, marker 永遠唔 render. Fix: testing-page.js line 1500 + 1650 之後加 `window.lastKlines = klines` + `window.lastChartRefs = chartRefs` sync 落 window global object, 對齊 line 1653-1655 既有 pattern (window.currentVerdict + window.currentKlines + window.currentChartRefs 已 set). 凡人話 verify (對齊 9月6日 16:47 永久 rule): 大少 hard reload testing page + 撳跑 M1 (HK.00700) + Tab B 揀「強下跌週期」, console.log 不再印 `lastChartRefs 或 lastKlines 缺失`, 改印 `[renderBrackTestChartOverlay] reuse handle.setMarkers, set 125 markers, handle.markers.length=125 (activeCycle=strong_downtrend)`, 肉眼 verify 125 條深紅 circle marker + 黑色 outline + size 2 + text 「強下跌 #1」 etc 同步 render 喺 chart 入面. 對齊 §M3 trendline chart overlay 修復永久 rule (9月6日 16:47) — render function 永遠拎 verdict.points + 肉眼 verify chart overlay 視覺效果; §M1 sub-scenario 永久 rule「改任何 sub_scenario display 都要即刻 update spec doc」spec doc §7 Chart top banner 更新 + Change log v0.1.0-fix10 entry; 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.190 → ?v=2.3.191 + 對齊 §Backend hot-reload 永久 rule (8月31日 11:01) 改 backend algo 必 restart + curl verify (backend 唔改所以唔需要); 凡人話 (8月14日 19:02 trigger) + 「用『取』唔用『拎』」(8月20日 trigger); 對齊 §M2 self-check penalty 永久 rule spirit「critical bug fix 必先 present fix 結果 + evidence + 等 trigger commit」+ Mavis 自己行 (9月10日 23:06) — 今次 fix 對齊呢個 pattern 因為 plan v0.1.0 fix7 + fix8 + fix9 都冇 fix 呢個 root cause, 大少 screenshot + console 揭發
...
// 大少 2026-09-15 06:29 — Brack Test Tab 順序對調 + 預設 cycle「強上升」(大少 06:29 trigger「brack test結果的 TAb 我想先放"🎯 按 sub-scenario 揀", 之後才到"📅 按時間排", 預設是🎯 按 sub-scenario 揀」). 改 4 個地方: (a) adapter.mjs renderBrackTestCard Tab HTML 順序對調 (line 5780-5783): 「🎯 按 sub-scenario 揀」Tab 放第一個 + 默認 active; (b) adapter.mjs Dropdown 默認 selected 第一個 cycle「🟢 強上升」 (line 5789); (c) adapter.mjs _brackTestRunHandler defaultActiveCycle = 'strong_uptrend' (line 6102-6127), renderBrackTestHitTable / renderBrackTestFilterInfo / renderBrackTestChartOverlay / renderBrackTestChartBanner 全部 default 拎 defaultActiveCycle; (d) testing-page.js init chart banner defaultBannerCycle = 'strong_uptrend' (line 1681-1696). 凡人話: 大少撳跑 Brack Test 第一眼見到 Tab B active + Dropdown 默認「強上升」+ Banner 顯示「強上升」+ Chart 入面深綠 marker, 而唔係 11 種顏色全部 + banner hidden. 對齊 §M1 sub-scenario 永久 rule (8月16日 19:21)「改任何 sub_scenario display 都要即刻 update spec doc」; 凡人話 (8月14日 19:02 trigger) + 「用『取』唔用『拎』」(8月20日 trigger); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.195 → ?v=2.3.196 + 對齊 §Backend hot-reload 永久 rule (8月31日 11:01) 改 backend algo 必 restart + curl verify (backend 唔改所以唔需要); Mavis 自己行 (9月10日 23:06) — 改 UI 默認值 後 commit
// 大少 2026-09-15 06:35 — Brack Test cycle dropdown 第一次開啟唔見 fix (大少 06:35 trigger「在"🎯 按 sub-scenario 揀"的第一次開啟時消失了"sub-scenario"的select list, 你去修正這問題」). Root cause: v0.2.1 改 Tab order 但漏 toggle dropdown visible (HTML 默認 `style="display:none;"`, 因為 `_brackTestRunHandler` 唔 call `_brackTestModeHandler` 嚟 trigger dropdown toggle, 所以撳跑 Brack Test 第一眼見到 dropdown 永遠唔見). Fix (adapter.mjs line 6110-6120): inline toggle dropdown visible + toggle mode-tab active class 對齊 cycle. 對齊 §M1 sub-scenario 永久 rule (8月16日 19:21)「改任何 sub_scenario display 都要即刻 update spec doc」; §K-line Cache 永久 rule — 唔重複 call `_brackTestModeHandler` 重 render data; 凡人話 (8月14日 19:02 trigger) + 「用『取』唔用『拎』」(8月20日 trigger); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.196 → ?v=2.3.197 + 對齊 §Backend hot-reload 永久 rule (8月31日 11:01) 改 backend algo 必 restart + curl verify (backend 唔改所以唔需要); Mavis 自己行 (9月10日 23:06) — fix bug 後 commit
...
const ALGO_CACHE_BUST = '5.4.27';  // 大少 2026-09-18 00:58 — Fix Brack Test A/B 對比 testing root cause (大少 forward console log evidence 揭發指定日期「執行」冇 render, 全跑「🎯 跑 Brack Test」有 render): (a) **Root cause** `_brackTestRunDateRangeHandler` (指定日期) 撳完 trigger `_runAlgorithmWithDateRange` → testing-page.js `runAlgorithm()` line 1702 `brackTestPanel.innerHTML = renderBrackTestCard(verdict)` rebuild inner HTML (用 `Math.random()` 新 panelId), handler 拎返舊 panel reference (`brack-test-panel-OLDID`) 已經 detached, render 落 detached element → user 睇唔到; 而 `_brackTestRunHandler` (全跑) 直接 fetch + render, panel 冇被 rebuild → render 落 active DOM ✅; (b) **Frontend** `adapter.mjs` `_renderBrackTestVerdict` 開頭加 detached panel fallback (用 `panel.isConnected` detect, fallback 去 query `#brack-test-panel` 固定 container 嘅 latest `.brack-test-card`), 對齊 DRY spirit (唔需要改 `runAlgorithm` 嘅 logic, runAlgorithm 嘅 rebuild 對全跑 user flow 仍然 work) + 對齊 §M3 trendline chart overlay 修復永久 rule (9月6日 16:47) silent return 唔 throw spirit; (c) **凡人話 verify** (對齊 §M3 永久 rule 凡人話肉眼 verify spirit): 大少 hard reload testing page (`?v=2.3.222`) → console.log 應該印 `panel.isConnected=true` (全跑 path, 唔 trigger fallback) + 撳指定日期「執行」之後 console.log 應該印 `⚠️ panel detached (isConnected=false), fallback 去 container latest .brack-test-card, newId=brack-test-panel-NEWID` + result table render 落 visible DOM ✅; (d) 對齊 §Cache bust self-check 永久 rule 21:24 sync bump (5.4.26 → 5.4.27 + ?v=2.3.221 → ?v=2.3.222).  // 大少 2026-09-18 00:24 — Fix ReferenceError: mini is not defined (大少 forward console log 「testing-page.js?v=2.3.215:955 Failed to load AS-03-MA: ReferenceError: mini is not defined at adapter.mjs?v=5.4.20:5755:25」): (a) **Root cause** adapter.mjs line 5730 嘅 `BRACK_TEST_PANEL_STYLE` 用 backtick template literal 包住成段 CSS, 我之前 v0.9.0 改動喺 line 5755 加 CSS comment 入面用咗 `\`.breakdown-mini\`\`.brack-breakdown-grid\`` backtick 字符 (markup 字眼), template literal 喺 column 24 個 `\`` closed 咗, 後面 `breakdown-mini` 入面個 `mini` 變 JavaScript undefined identifier (template literal 內部 `${expression}` syntax 但冇 expression 直接拎 variable 名 → ReferenceError). 大少 forward console log 揭發 10 個 AS-03 module 全部 init fail (testing page 完全 render 唔到); (b) **Frontend** `adapter.mjs` line 5755 拎走 `\``\`.breakdown-mini\`\``\`.brack-breakdown-grid\``\`` 嘅 backtick 字符, 改用普通字 `breakdown-mini` / `brack-breakdown-grid` + 加 `⚠️ 凡人話警告: 呢段 CSS 喺 BRACK_TEST_PANEL_STYLE template literal (line 5730 用 backtick 包), 唔可以用 backtick 字入 comment (會 close template literal → ReferenceError)` 永久 warning comment; (c) **凡人話 verify** (對齊 §M3 trendline chart overlay 修復永久 rule 9月6日 16:47 凡人話肉眼 verify spirit): 大少 hard reload testing page (`?v=2.3.216`) → console.log 應該冇 `ReferenceError: mini is not defined` error + 10 個 AS-03 module (AS-03-MA/HL/TL/IND/VP/VOL/SYN/BT/DEC/BTL) 全部 init success. (d) 對齊 §Cache bust self-check 永久 rule 21:24 sync bump (5.4.20 → 5.4.21 + ?v=2.3.215 → ?v=2.3.216).  // 大少 2026-09-18 00:14 — Brack Test card 補回 State 分佈 + cycle breakdown 6 個獨立 box grid (對齊 image 2 紅框內嘅 6 個獨立 box 排版, v0.9.0 對齊大少 23:59 trigger「backtest 修正, 在指定的日期內跑, 也要有『按 sub-scenario 揀』和 sub-scenario 選項, 『📅 按時間排』」實際係指 Brack Test card 而唔係 M9 Back Test): adapter.mjs renderBrackTestSummary 拆 2 個 function (renderBrackTestSummary 拎 summary 文字 + renderBrackTestBreakdownGrid 拎 grid HTML); renderBrackTestBreakdownGrid 拎走 v0.2.1 `.breakdown-mini` inline 寫法, 改用 `.brack-breakdown-grid` 6 個獨立 box (1 個 State 分佈 box + 5 個 cycle 獨立 box) 對齊 image 2 紅框內嘅 6 個獨立 box 排版; CSS `.brack-breakdown-grid` 加 `display:grid; grid-template-columns:repeat(auto-fill, minmax(180px, 1fr))` + `.brack-breakdown-box` 加 `background:#fff3e0; border:1px solid #ffcc80; border-radius:6px` + `.brack-breakdown-state` 加 `grid-column:span 2` (State 分佈 box 跨 2 column 對齊 image 2 排版); _renderBrackTestVerdict 分別 query `.brack-summary` + `.brack-breakdown` element inject (對齊 §Backend 永久改 emit field name 之後 frontend 必先 grep 全 reference 對齊永久 rule 9月10日 23:45 spirit). 同時 Revert 我之前嘅 M9 Back Test v0.7.0 改動 (理解錯嘅 task): backend api/algorithms.py 拎走 sub_scenarios + sort_by 2 個 Query params + options dict 注入; backend back_test/algorithm.py 拎走 sub-scenario filter + sort by date 邏輯 + run_walk_forward_cv + BackTestAlgorithm.run() 拎走 subScenarios + sortBy 落 validate_replay_config; adapter.mjs backTestAdapter.inputs 拎走 4 個新 fields (dateFrom / dateTo / subScenarios / sortBy) + analyze URLSearchParams 拎走 4 個新 params + 拎走 spread frontendShape + renderResult Section 1 header 拎走 v0.7.0 inline summary banner; testing-page.js 拎走 3 個 render function (renderDate + renderRadio + renderMultiSelectChips) + renderInput switch 拎走 3 個 case; docs/research/AS-03-cycle-detection/MODULE-09-BACK-TEST.md 拎走 §15 v0.7.0 section; docs/research/AS-03-cycle-detection/MODULE-BRACK-TEST.md 新加 v0.9.0 section. §Array 邏輯必先 curl evidence 確認排法 永久 rule (8月31日 13:14) — backend `m1_brack_test/algorithm.py` emit 11 個 cycle count, filter value 0 + sideways 拎返 5 個 cycle (對齊 image 2 紅框內嘅 5 個 cycle box display). §M1 sub-scenario 永久 rule (8月16日 19:21) — cycle breakdown display 改動即 update spec doc.  // 大少 2026-09-17 13:38 — Brack Test chart marker label 對齊例表 Index (v0.8.0, 對齊大少 trigger 9月17日 13:38 「強上升週期 #208 應該要對應返 BrackTest 結果例表裡的 Index」): 凡人話 fix (對齊 plan spec §3 path MODULE-BRACK-TEST.md §4.2 + AGENTS.md 永久 rule section), 詳見 spec doc §4.2 v0.8.0 section. // 大少 2026-09-17 13:21 — M1 強升 / 強跌 trigger 加 K 線 high/low 確認 (v2.7.0): backend algorithm.py Priority 2 (line 528-548) + Priority 3 (line 591-609) 加 2 條 K 線 high/low 比較條件 (今日 vs 昨日),對齊大少 9月17日 13:21 trigger「例改 sub-scenario 強升的算法,加入兩個新規則,今日的 High > 昨日的 High and 今日的 Low > 昨日的 Low,強跌也要有相應的規則修改」; frontend adapter.mjs BRACK_TEST_CYCLE_EXPLANATIONS strong_uptrend (line 6114) + strong_downtrend (line 6117) 加凡人話 trigger 條件 (對齊 Brack Test banner ⓘ tooltip 永久 rule 2026-09-17 08:30); strategyStrongUp (line 1672) + strategyStrongDown (line 1678) 「訊號確認」凡人話 trigger 條件. Cache bust self-check 永久 rule 21:24 sync bump (5.4.17 → 5.4.18 + ?v=2.3.212 → ?v=2.3.213). Backend hot-reload 永久 rule 8月31日 11:01 (改 backend algo 必 restart + curl verify). 對齊 §sub_scenario 流程永久 rule 8月16日 19:21 — sub_scenario 改動要拎 ≥ 3 stock A/B test verify. Spec doc M1-V22-RESEARCH.md 強升 row line 338 + 強跌 row line 339 update + 🟢 大少 trigger #N+9 section.  // 大少 2026-09-17 08:30 — Brack Test cycle tooltip 改寫成「直接簡單算法」trigger 條件 (對齊 backend algorithm.py line 477-680 嘅 11 個 elif trigger 條件, 等大少拎去參考微調 algorithm): BRACK_TEST_CYCLE_EXPLANATIONS 11 個 entry 由抽象嘅「Zmen X rule + Layer 2 Y」寫法改為直接列出 trigger 條件 (排列 / 斜率正負 / P 點方向 / 拎幾多個 P 點), 大少撳 banner ⓘ icon 嗰陣即刻見到呢個 cycle 嘅 trigger 條件, 等佢可以拎去微調 algorithm. 對齊 §M1 sub-scenario 永久 rule (8月16日 19:21) sub_scenario display 改動即 update spec doc (§7 + Change log v0.7.0 entry). Cache bust self-check 永久 rule 21:24 sync bump (5.4.16 → 5.4.17 + ?v=2.3.211 → ?v=2.3.212). Backend 唔需要改 (純 frontend display 改動).  // 大少 2026-09-16 21:15 — v0.5.7 拎走 v0.5.4 嘅 from + to auto-clamp logic (大少 21:15 reject v0.5.6「點解你改來改去做係有問題， 還是『多一日』的功能不能用」 + console log evidence 「from (2010-01-01) 早過 K 線第一日 (2021-06-22), auto-clamp → 2021-06-22」): v0.5.6 嘅 Step 3 已係最簡單邏輯 (只 modify to), 但 v0.5.4 嘅 Step 5 auto-clamp logic 仲 trigger from modify (因為 user input date 越界 K 線 first). Fix: 拎走 v0.5.4 嘅 from + to auto-clamp logic. 兩個制都係 modify to date, from 永遠唔變, Backend KlineCache fetch K 線會自動用 K 線 actual range (越界 date 唔影響 verdict). 對齊 §Config UX 模式 spirit (2026-08-19 13:03) — user 揀過嘅 value 永遠要保留, 唔好 auto-clamp date input. 對齊 §Backend hot-reload 永久 rule 8月31日 11:01 + cache bust self-check 永久 rule 21:24 (5.4.15 → 5.4.16 + ?v=2.3.210 → ?v=2.3.211). 凡人話 verify (testing page reload + 撳跑 + date inputs 揀 [2010-01-01, 2026-08-06] → 撳 ▶ +1 日 → console.log 印 `re-run: new from=2010-01-01, new to=2026-08-07`, date input to 變 2026-08-07, from 仲係 2010-01-01, re-run K 線 + ZigZag + P 點 + Brack Test verdict).  // 大少 2026-09-16 20:59 — v0.5.6 簡化 v0.5.5 單邊 modify (大少 20:59 reject v0.5.5「你很差啊， 現在加一日和減一日都用不了， 其他可以簡單處理， 你只要吧 To Date 改變一日， 然後再跑一次就可以了」): 兩個制 (◀ -1 日 / ▶ +1 日) 都係 modify to date 一日, from 永遠唔變, 最簡單 logic. 對齊 §Backend hot-reload 永久 rule 8月31日 11:01 + cache bust self-check 永久 rule 21:24 (5.4.14 → 5.4.15 + ?v=2.3.209 → ?v=2.3.210). Frontend only fix (adapter.mjs _brackTestShiftDateHandler Step 3 logic 簡化). 凡人話 verify (testing page reload + 撳跑 + date inputs 揀 [2000-01-01, 2026-08-07] → 撳 ◀ -1 日 → date input to 變 2026-08-06, from 仲係 2000-01-01, re-run; 撳 ▶ +1 日 → date input to auto-clamp 2026-08-07, from 仲係 2000-01-01, re-run).  // 大少 2026-09-16 20:50 — v0.5.5 ±1 日制改單邊 modify (大少 20:50 trigger「現在減一日是對的， 但加一日是錯的， 你是修改了 From Date， 應該是修改 To Date」): ◀ -1 日 = extend left boundary (modify from only, to 唔變), ▶ +1 日 = extend right boundary (modify to only, from 唔變). v0.5.4 ±1 日制都係 from + to 雙邊 modify, 大少 accept -1 日但 reject +1 日 (expect 只 modify to). Fix: adapter.mjs _brackTestShiftDateHandler Step 3 logic 改單邊 modify. Auto-clamp v0.5.4 spirit 保留: ◀ -1 日如果 from 越界 firstKlineDate → auto-clamp + re-run, ▶ +1 日如果 to 越界 lastKlineDate → auto-clamp + re-run. Edge case (d) -1 日 dateFrom empty / +1 日 dateTo empty → silent warn + return. 對齊 §Backend hot-reload 永久 rule 8月31日 11:01 + cache bust self-check 永久 rule 21:24 (5.4.13 → 5.4.14 + ?v=2.3.208 → ?v=2.3.209). 凡人話 verify (testing page reload + 撳跑 + date inputs 揀 [2000-01-01, 2026-08-07] → 撳 ◀ -1 日 → console.log 印「from auto-clamp → 2021-06-23」, date input from 變 2021-06-23, to 仲係 2026-08-07 + re-run; 撳 ▶ +1 日 → console.log 印「to auto-clamp → 2026-08-07」, date input from 仲係 2021-06-23, to 仲係 2026-08-07 + re-run).  // 大少 2026-09-16 20:39 — v0.5.4 boundary check 改 auto-clamp + 永遠 re-run (大少 20:39 trigger「我指定的Bracktest 日期是 2000-1-1 至2026-8-7， 但console log: [Brack Test ±1 日] 撳完後 from (1999-12-31) 早過 K 線第一日 (2021-06-23), 唔 trigger. 這個問題在時間上是全錯了，你要找回當時K線的時間Range 才可以做到加一日或減一日」): K 線 first date / last date 係 authoritative source, user 輸入 date 越界 → auto-clamp 落 K 線 first / last date, ±1 日永遠 re-run. Edge case (b) from > to (極端 case) 保留 silent warn + return. Frontend only fix (adapter.mjs _brackTestShiftDateHandler line 6448+ boundary check 邏輯改 auto-clamp). 對齊 §Backend hot-reload 永久 rule 8月31日 11:01 + cache bust self-check 永久 rule 21:24 (5.4.12 → 5.4.13 + ?v=2.3.207 → ?v=2.3.208). 凡人話 verify (testing page reload + 撳跑 → 撳 ±1 日制 → 肉眼 verify date input auto-clamp 落 K 線範圍 + re-run K 線 + ZigZag + P 點 + Brack Test verdict).  // 大少 2026-09-16 17:23 — Brack Test 指定日期 ±1 日快速調整 UI (v0.5.3): 「執行」制右邊加「◀ -1 日」+「+1 日 ▶」2 個掣, 撳完自動重跑 K 線 + ZigZag + P 點 + Brack Test verdict, 對齊 v0.5.2 pattern. Frontend only fix (adapter.mjs `renderBrackTestCard` 加 2 個 button + 新加 `_brackShiftDate` helper + 新加 `window._brackTestShiftDateHandler(panelId, symbol, delta)` 共用 handler). Backend 唔需要 restart. 對齊 §Backend hot-reload 永久 rule 8月31日 11:01 + cache bust self-check 永久 rule 21:24 (5.4.11 → 5.4.12 + ?v=2.3.206 → ?v=2.3.207). 凡人話 (testing page reload + 撳跑 → 肉眼 verify); 大少 2026-09-16 06:16 — Brack Test 指定日期範圍 backend runner 拎 caller start/end (v0.4.4 修正 v0.4.3 漏網之魚): backend services/algorithm_runner.py line 125-137 拎 caller 嘅 dateFrom + dateTo 從 options.get(...) 覆蓋 start_date + end_date (silent fallback: caller 冇傳 → 用既有 today - calendar_days_back default 對齊 §M3 永久 rule spirit「silent return 唔 throw」); frontend 不需要改 (v0.4.3 commit 已經傳 start/end); cache bust 5.4.7 → 5.4.8 + ?v=2.3.202 → 2.3.203 對齊 21:24 cache bust self-check 永久 rule + 大少 22:27 trigger fetchBackendZigZag 加 dateFrom + dateTo + dataWindowDays optional args, fetch URL add date_from/date_to params (對齊 backend api/algorithms.py start/end 既有 pattern) + data_window_days 用 caller 嘅 filtered K 線 length (對齊 K-line Cache 永久 rule spirit 8月22日 23:20「Frontend 拎 data, Backend 拎 K 線」); fetchAndInjectBackendZigZag 加 3 個 args + 傳落 fetchBackendZigZag; runAlgorithm line 1599 call site 加 3 個 args (從 runAlgorithm scope 拎 dateFrom/dateTo/klines.length); audit 其他 chart overlay 來源 (M1 verdict / Brack Test verdict / chart banner / cycle legend / MA 線 / 鮮紫觸發點 marker / 鮮綠 extension line / Brack Test cycle marker) 全部對齊 filtered K 線 ✅, 只有 ZigZag 漏咗; cache bust 5.4.6 → 5.4.7 + ?v=2.3.201 → 2.3.202 對齊 21:24 cache bust self-check 永久 rule + 大少 21:37 trigger testing-page.js runAlgorithm 加 optional dateFrom/dateTo args + fetch K 線嗰陣 add `start` + `end` query params (對齊 backend api/kline.py line 47 既有 pattern + KlineCache `get_or_fetch` line 144-145 拎 filtered K 線); expose `window._runAlgorithmWithDateRange = function(dateFrom, dateTo) { return runAlgorithm(dateFrom, dateTo); }` 對齊 adapter.mjs trigger 透過 window global; adapter.mjs `_brackTestRunDateRangeHandler` 重寫成 trigger `window._runAlgorithmWithDateRange(dateFrom, dateTo)`, 唔再自己 fetch verdict + render (對齊大少 21:37 trigger「整個 K 線圖 + Brack Test 都按指定日期重跑」); Revert backend m1_brack_test/algorithm.py v0.4.0 改動 (拎走 `_parse_date_range` + `_kline_date_ts` + loop filter), 對齊 §K-line Cache 永久 rule spirit「Frontend 拎 data, Backend 拎 K 線」; 凡人話 verify scope 同 §M3 trendline chart overlay 修復永久 rule (9月6日 16:47) 凡人話肉眼 verify spirit (大少 hard reload testing page + 撳 date inputs 揀 [2026-09-01, 2026-09-15] + 撳「執行」button → K 線圖 K 線只 render 範圍內 + chart overlay 全部 reset + M1 verdict + Brack Test verdict + chart banner + cycle legend 全部 reset); cache bust 5.4.5 → 5.4.6 + ?v=2.3.200 → 2.3.201 對齊 21:24 cache bust self-check 永久 rule + 大少 21:05 trigger adapter.mjs renderBrackTestCard 加 .brack-run-row 結構 (保留左邊 button + 右邊 date_from/date_to inputs + 「執行」button); 新加 _brackTestRunDateRangeHandler window function 共用 _renderBrackTestVerdict render helper; BRACK_TEST_PANEL_STYLE 加 .brack-date-input + .brack-run-date-range-btn CSS; backend api/algorithms.py 加 date_from/date_to Query params + options dict 注入 dateFrom/dateTo; backend m1_brack_test/algorithm.py 加 _parse_date_range + _kline_date_ts helper + loop date range filter; cache bust 5.4.4 → 5.4.5 + ?v=2.3.199 → 2.3.200 對齊 21:24 cache bust self-check 永久 rule + §Backend hot-reload 永久 rule (改 backend 必 restart + curl verify); 大少 06:55 trigger testing-page.js line 1696 chart banner init 加 guard `if (verdict && verdict.points && verdict.points.length > 0)` 先 render banner, 否則 innerHTML = '' (hidden); adapter.mjs renderBrackTestChartBanner line 6096 加 defensive guard `if (hits.length === 0) return '';` 對齊 §M3 trendline chart overlay 修復永久 rule (9月6日 16:47) spirit「silent return 唔 throw」, 涵蓋 updateBrackTestChartBanner + _ModeHandler / _CycleHandler 等所有 callers; 凡人話 verify scope 同 §M3 永久 rule 凡人話肉眼 verify spirit (大少 hard reload testing page + 撳跑 M1 但唔撳 Brack Test → chart banner 唔顯示); cache bust 5.4.3 → 5.4.4 + ?v=2.3.198 → 2.3.199 對齊 21:24 cache bust self-check 永久 rule (大少 06:45 trigger「在 Bracktest 結果裡的『按 sub-scenario 揀』Tab, 揀指定 sub-scenario 嗰陣, 喺 K 線圖表上方顯示嗰個 sub-scenario banner」): adapter.mjs 新加 renderBrackTestChartBanner(verdict, activeCycle) helper (對齊 line 5919-5928 renderBrackTestFilterInfo pattern, 但用 cycle 顏色 background + 白字, activeCycle='all' 返 empty string) + updateBrackTestChartBanner(verdict, activeCycle) helper (拎 #brack-chart-banner DOM + populate innerHTML) + 修改 _ModeHandler / _CycleHandler 2 個 handler 同步 call updateBrackTestChartBanner (對齊 line 6055-6066 / 6076-6086 pattern); BRACK_TEST_PANEL_STYLE 加 .brack-chart-banner CSS (cycle color background + 白字 + 圓角 + box-shadow 對齊 Futu health banner style spirit); module 尾 `export { renderBrackTestCard, renderBrackTestFilterInfo, renderBrackTestChartBanner }` 加返 renderBrackTestChartBanner named export (對齊之前 renderBrackTestCard / renderBrackTestFilterInfo SCOPE fix pattern 21:35 / 22:22); testing-page.js 加 lastBrackChartBannerVerdict global state 對齊 lastChartRefs pattern (line 783-784) + runAlgorithm handler 加 initBrackChartBanner 對齊 M6 dashboard panel pattern (line 1555-1574, chart-section 入面 chart-container 之前 conditional render) + resetResultPanel 加清 brack-chart-banner (對齊 m6-dashboard-panel pattern); index.html 加 `<div id="brack-chart-banner"></div>` 喺 chart-container 之前 + ?v=2.3.187 → ?v=2.3.188 sync bump 對齊 21:24 cache bust self-check 永久 rule; 對齊 §M1 sub-scenario 永久 rule「改任何 sub_scenario display 都要即刻 update spec doc」spec doc §7 Chart top banner 新加 + Change log v0.1.0-fix7 entry; backend 唔需要改 (純 frontend UI 改動); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.187 → ?v=2.3.188 + 對齊 §Backend hot-reload 永久 rule (8月31日 11:01) 改 backend algo 必 restart + curl verify (backend 唔改所以唔需要)
...
// 大少 9月2日 00:52 trigger (4.66.0) — 拎返 P 點 + 鮮紫觸發點 marker toggle state (4.53.0 拎走嘅 spirit 拎返, 預設關)
...
// 大少 2026-09-14 23:18 trigger — Brack Test chart top banner global state
// 凡人話: 用嚟畀 adapter.mjs _ModeHandler / _CycleHandler 拎返 panel + verdict + activeCycle 同步 update chart banner
// 對齊 lastChartRefs pattern (line 783-784), global state 跨 handler 共享
let lastBrackChartBanner = null;  // { panel: HTMLElement, verdict: object, activeCycle: string }
// 大少 2026-09-15 06:45 trigger — Brack Test hit row click delegation attach flag (ensure 只 attach 1 次, 避免多次 runAlgorithm 重複 trigger pan/zoom)
let brackTestRowClickAttached = false;
...
function resetResultPanel() {
  ...
  // 大少 9月14日 23:18 trigger — 清 chart banner (對齊 m6-dashboard-panel pattern, 切換 algo / 撳跑新 algo 唔殘留舊 banner)
...
// 大少 2026-09-15 21:37 trigger — Brack Test 指定日期範圍完整重跑 (v0.4.1)
// 凡人話: 大少 trigger「整個 K 線圖和 Brack Test 都按指定的日期內重新再跑」, 撳「執行」button 之後重新 fetch K 線 (filtered by date_from ~ date_to) + 重新 render chart + 重新 fetch M1 verdict + 重新 fetch Brack Test verdict
// runAlgorithm 拎 optional dateFrom / dateTo args, fetch K 線嗰陣 add `start` + `end` query params (對齊 backend api/kline.py line 47 既有 pattern + KlineCache `get_or_fetch` line 144-145 拎 filtered K 線)
// 對齊 §K-line Cache 永久 rule (8月22日 23:20) — K 線 filtered 喺 K-line Cache layer
// 對齊 §M3 trendline chart overlay 修復永久 rule (9月6日 16:47) — silent return 唔 throw, 凡人話肉眼 verify
async function runAlgorithm(dateFrom, dateTo) {
  if (!currentAdapter) {
    runStatus.innerHTML = '❌ 未揀演算法, 請先去上面揀一個';
    return;
  }
  // 大少 21:37 trigger — silent fallback (dateFrom / dateTo 都係 empty)
  dateFrom = dateFrom || null;
  dateTo = dateTo || null;
  if (dateFrom === null && dateTo === null) {
    // 全部 empty → 唔 trigger fetch (frontend validation, 由 adapter.mjs _brackTestRunDateRangeHandler 入面先做)
    // 因為 adapter.mjs 一早 silent warn, 唔會 call 到呢度, 但保留 safety net
  }
  ...
  try {
    ...
    // 大少 2026-09-15 21:37 trigger — Brack Test 指定日期範圍完整重跑 (v0.4.1): fetch K 線嗰陣加 `start` + `end` query params (對齊 backend api/kline.py line 47 既有 pattern + KlineCache `get_or_fetch` line 144-145 拎 filtered K 線)
    // 凡人話: dateFrom → start, dateTo → end; empty / undefined → 唔加 params, fallback 全 K 線
    // 對齊 §Cross-module 統一 date parsing 永久 rule (8月29日 22:35) — YYYY-MM-DD 格式
    const dateRangeParams = (dateFrom || dateTo)
      ? `${dateFrom ? `&start=${encodeURIComponent(dateFrom)}` : ''}${dateTo ? `&end=${encodeURIComponent(dateTo)}` : ''}`
      : '';
    const klineUrl = `${BACKEND_URL}/api/kline?code=${encodeURIComponent(code)}&period=${period}&count=${count}${dateRangeParams}`;
    ...
    if (!Array.isArray(klines) || klines.length === 0) {
      // 大少 2026-09-15 22:27 trigger — silent fallback (對齊 §M3 trendline chart overlay 修復永久 rule 9月6日 16:47 spirit「silent return 唔 throw」)
      // 凡人話: backend /api/kline 拎唔到 K 線 (cold cache / OpenD history 限制) → return empty klines array, frontend 應該 silent fallback + 顯示 friendly error message, 唔 throw console error
      const errorMsg = klineData.error || '後端 K 線冷啟動或 OpenD history 限制, 請 retry 或 check FutuOpenD connection';
      console.warn(`[runAlgorithm] ${errorMsg} (klines type=${typeof klines}, length=${Array.isArray(klines) ? klines.length : 'N/A'})`);
      runStatus.innerHTML = `⚠️ ${errorMsg} (請 retry 或 check FutuOpenD connection)`;
      resultPanel.innerHTML = `<div class="brack-error" style="background:#fff2f0;border:1px solid #ffccc7;border-radius:6px;padding:12px;color:#cf1322;">⚠️ ${escapeHtml(errorMsg)}<br/><br/>建議:<br/>1. Retry (撳多次「跑算法」)<br/>2. Check FutuOpenD 連線<br/>3. Check stock code 是否 valid</div>`;
      return;  // 大少 22:27 trigger — silent fallback 唔 throw, 對齊 §M3 永久 rule spirit
    }
    ...
    lastVerdict = verdict;
    lastKlines = klines;
    // 大少 2026-09-14 23:43 trigger — Brack Test chart marker 唔見嘅真正 root cause fix: testing-page.js 嘅 lastChartRefs + lastKlines 係 top-level let variable, 唔會自動 attach 落 window, 但 adapter.mjs _brackTestRunHandler (line 6079) 同 _brackTestCycleHandler (line 6084) 同 _brackTestModeHandler (line 6063) 都拎 window.lastChartRefs / window.lastKlines, 所以永遠係 undefined → renderBrackTestChartOverlay 嘅 if (chartRefs && klines && klines.length) 永遠 false → 唔 render markers。Fix: 同步 set 落 window, 對齊 line 1653-1655 既有 window.currentChartRefs / window.currentKlines pattern
    window.lastKlines = klines;
    ...
    if (currentAdapter.id === 'AS-03-MA') {
      ...
      // 已經 inject, 自動 render 紫線 + P 點 arrow marker + 紅色觸發點 circle (4.62.0 永久 rule)
      ...
      // 大少 2026-09-15 22:27 trigger — Brack Test 指定日期範圍完整重跑 (v0.4.3) 修 ZigZag 沒有重跑 issue
      // 凡人話: 將 runAlgorithm scope 嘅 dateFrom/dateTo/klines.length 傳落 fetchAndInjectBackendZigZag, ZigZag 紫線 + P 點 marker 對齊 filtered K 線 range
      // klines.length 係 filtered K 線 count, 對齊 backend data_window_days 拎 filtered ZigZag verdict
      await fetchAndInjectBackendZigZag(thresholdMode, manualThreshold, lookback, multiplier, dateFrom, dateTo, klines.length);
    }
    ...
    if (currentAdapter.renderResult) {
      ...
      // 大少 2026-09-10 23:45 — 頂部 banner normalize 拎 backend emit `verdict.warnings` 落 `_warnings`
      // 對齊 8月31日 Batch 2 永久 rule (512a2138): backend emit `warnings` 冇 underscore
      // 對齊 §Module Warning v1.0.0: frontend internal 用 `_warnings` 畀 UI helper (warnings.mjs) 拎
      // 凡人話: 之前 frontend 拎 `_warnings` 永遠 None (backend emit 喺 `warnings`),
      //         強跌/Hurst+ADX gate 等 stock_state banner 永遠唔出
      verdict._warnings = verdict.warnings || verdict._warnings || [];
    ...
    // 大少 2026-09-14 22:04 trigger — M1 Brack Test 移到 K線圖框內最下邊
    // 凡人話: 撳跑 M1 之後, 將 Brack Test card 寫入 #brack-test-panel (chart-section 入面, m6-dashboard-panel 之後, result section 之前)
    // 對齊 §M6 dashboard panel pattern (line 1555-1574), 大少撳跑 M1 視線一離開 chart 即刻見到 Brack Test 入口
    // 其他 algo 嗰陣清返 Brack Test panel (避免顯示 M1 舊 verdict)
    const brackTestPanel = document.getElementById('brack-test-panel');
    if (brackTestPanel) {
      if (currentAdapter.id === 'AS-03-MA' && currentAdapter.renderBrackTestCard) {
        try {
          const { renderBrackTestCard, renderBrackTestFilterInfo } = await import('../algorithms/AS-03-cycle-detection/adapter.mjs?v=' + ALGO_CACHE_BUST);
          brackTestPanel.innerHTML = renderBrackTestCard(verdict);
          // 大少 22:20 trigger — 撳跑 Brack Test 之後, 表格上方顯示 filter info (默認「全部 sub_scenario: X 條」)
          const filterInfo = brackTestPanel.querySelector('.brack-filter-info');
          if (filterInfo) filterInfo.innerHTML = renderBrackTestFilterInfo(verdict.points || [], 'all');
          // 大少 9月6日 16:47 永久 rule 沿用 (testing page chart overlay 視覺 verify): Brack Test 都要肉眼 verify
        } catch (err) {
          console.error('[M1 Brack Test] render failed:', err);
          brackTestPanel.innerHTML = `<div style="background:#fff2f0;border:1px solid #ffccc7;border-radius:6px;padding:12px;color:#cf1322;">⚠️ M1 Brack Test 渲染失敗: ${err.message}</div>`;
        }
      } else {
        // 撳跑其他 algo → 清 Brack Test panel
        brackTestPanel.innerHTML = '';
      }
    }
    ...
    // 凡人話: 撳跑完 algorithm 之之後, fetch /api/stocks/{code} 拎 stock name, 寫入 chart-header
    // 既唔需要 restart backend, 又唔會 block render (fetch 失敗 fallback 顯示 code only)
    updateStockNameDisplay(code);

    // 大少 2026-09-14 23:18 trigger — Brack Test chart top banner init (圖表上方顯示當前揀緊嘅 sub-scenario)
    // 大少 2026-09-15 06:29 trigger — 預設 activeCycle 由 'all' 改為 'strong_uptrend' (對齊大少 trigger「預設是🎯 按 sub-scenario 揀」+ Tab B 默認 active + Dropdown selected 第一個 cycle)
    // 凡人話: 撳跑 M1 嗰陣, chart 上面 init chart banner element (activeCycle='strong_uptrend' 預設顯示, 因為 Tab B「按 sub-scenario 揀」+ Dropdown 第一個 cycle 「強上升」都係默認)
    // 對齊 M6 dashboard panel pattern (line 1555-1574), 其他 algo 嗰陣清返 chart banner
    // 對齊 §Backend 永久改 emit field name 之後, frontend 必先 grep 全 reference 對齊 (9月10日 23:45 trigger) — banner 用 backend emit cycle / cycleLabel 對齊 §verdict.meta.symbol 永久 rule (9月7日 08:30)
    // 對齊 9月7日 21:50 trigger「凡新加 render function 必 escape HTML」 — _brackEscapeHtml 對齊 renderBrackTestFilterInfo pattern
    const brackChartBannerEl = document.getElementById('brack-chart-banner');
    const defaultBannerCycle = 'strong_uptrend';  // 大少 06:29 trigger — 預設 banner 顯示第一個 cycle「強上升」
    if (brackChartBannerEl) {
      if (currentAdapter.id === 'AS-03-MA') {
        try {
          const { renderBrackTestChartBanner } = await import('../algorithms/AS-03-cycle-detection/adapter.mjs?v=' + ALGO_CACHE_BUST);
          // 大少 2026-09-15 06:55 trigger — fix 未跑 Brack Test 唔顯示 misleading banner
          // 凡人話: 撳跑 M1 但仲未撳跑 Brack Test 嗰陣, verdict.points 唔存在 (M1 verdict 唔等於 Brack Test verdict), 唔應該顯示「(0 條 / 全部 0 條)」banner
          // 加 guard: verdict.points populated (length > 0) 先 render banner, 否則 innerHTML = '' (hidden) 對齊既有 catch block 嘅 fallback pattern
          if (verdict && verdict.points && verdict.points.length > 0) {
            brackChartBannerEl.innerHTML = renderBrackTestChartBanner(verdict, defaultBannerCycle);
            lastBrackChartBanner = { panel: brackChartBannerEl, verdict, activeCycle: defaultBannerCycle };
          } else {
            brackChartBannerEl.innerHTML = '';
            lastBrackChartBanner = null;
          }
        } catch (err) {
          console.error('[Brack Test chart banner] render failed:', err);
          brackChartBannerEl.innerHTML = '';
          lastBrackChartBanner = null;
        }
      } else {
    ...
    // 大少 2026-09-15 06:45 trigger — Brack Test hit row click delegation (撳 hit row 日期 → K 線圖 pan/zoom 到 hit.date 中間 + 3 個月)
    // 凡人話: 撳 hit row 嘅任何 cell (尤其係日期 cell) → K 線圖即時 pan + zoom, 撳整行都 trigger (Option 3 大少 confirm)
    // Click delegation 對 #brack-test-panel 入面 <tr data-hit-date> 做, 對齊 testing-page.js line 1411 既有 document.addEventListener pattern 但 element-scoped (避免 global pollution)
    // brackTestRowClickAttached flag 確保只 attach 1 次 (避免多次 runAlgorithm 重複 trigger), 切其他 algo → innerHTML 清 DOM 但 event listener 唔移除, handler 拎唔到 <tr data-hit-date> silent return
    if (!brackTestRowClickAttached && brackTestPanel) {
      brackTestRowClickAttached = true;
      const adapterModule = await import('../algorithms/AS-03-cycle-detection/adapter.mjs?v=' + ALGO_CACHE_BUST);
      brackTestPanel.addEventListener('click', (e) => {
        // 凡人話: 拎 chart + klines 從 window (對齊 line 1752-1754 既有 fix 10 pattern, window.lastChartRefs + window.lastKlines sync)
        const chartRefs = window.lastChartRefs;
        const klines = window.lastKlines;
        adapterModule._brackTestRowClickHandler(e, chartRefs, klines);
      });
    }
    ...
    // 大少 2026-09-14 23:43 trigger — 對齊 line 1502 fix (lastKlines 同步 set 落 window): chart refs 同步 set 落 window, 對齊 line 1655 既有 window.currentChartRefs = chartRefs pattern (window.lastChartRefs = chartRefs 同步畀 adapter.mjs _brackTestRunHandler + _ModeHandler + _CycleHandler 拎到)
    window.lastChartRefs = chartRefs;
...
function renderChart(klines, code, period) {
  ...
  // Create chart
  // 大少 2026-09-09 09:32 — M4 multi-pane 副圖: createChart options 加 layout.panes defaults
  // 凡人話: enableResize true + separatorColor 預設值, 之後 M4 撳跑嗰陣 renderIndicatorsChartOverlay 會
  // 自動加 Pane 1 (RSI) / Pane 2 (MACD) 並共用同一個 time scale
  // 對齊 LWC v5 panes 永久 rule: chart.panes() 預設 share 同一個 time scale, 凡人話 pan/zoom 自動同步
  ...
  return { chart, candleSeries, priceLines: {}, chartContainer: container };  // 大少 2026-09-09 21:50 Spec Sync #59 Option C: chartContainer 畀 chart overlay legend box 用
}

// ===== Event listeners =====

algorithmSelect.addEventListener('change', onAlgorithmChange);
// 大少 2026-09-16 07:27 trigger — Run Algorithm button event-leak bug fix: arrow function wrap 避免 PointerEvent 漏入 runAlgorithm(dateFrom, dateTo) 第一個 arg → backend log 印 `start=[object PointerEvent]` → backend 拎唔到 K 線 → frontend silent fallback error. Root cause (curl + backend log evidence): `runBtn.addEventListener('click', runAlgorithm)` 撳 button 嗰陣 event listener 默認傳 `(event)` 做 first arg, runAlgorithm signature 第一個 param `dateFrom = PointerEvent` (truthy object), dateRangeParams 變咗 `&start=[object PointerEvent]`. 凡人話 (對齊 §M3 trendline凡人話肉眼 verify 9月6日 16:47): 大少 hard reload testing page + 撳 M1 + 撳 Brack Test 指定日期跑 → 撳「執行」OK → 撳返「跑算法」button → verdict 正常 render (之前 backend log 印 start=[object PointerEvent] + 返 0 條 K 線); §Backend hot-reload 永久 rule (8月31日 11:01) — frontend only fix, backend 唔需要 restart; 永久 rule (新加 AGENTS.md): 「Frontend button event listener 永遠 wrap arrow function 避免 event object 漏入 function args」; 跟 cache bust self-check 永久 rule 21:24 sync bump ALGO_CACHE_BUST 5.4.9 → 5.4.10 + ?v=2.3.204 → 2.3.205
runBtn.addEventListener('click', () => runAlgorithm());
// 大少 2026-09-15 21:37 trigger — Brack Test 指定日期範圍完整重跑 (v0.4.1) — expose 落 window global 畀 adapter.mjs _brackTestRunDateRangeHandler trigger
// 凡人話: testing-page.js 同 adapter.mjs 係 separate module, adapter.mjs 透過 window._runAlgorithmWithDateRange trigger 拎 K 線 (filtered) + renderChart reset + 重新 fetch verdict + render
// 對齊既有 window._brackTestRunHandler pattern (adapter.mjs 透過 window global expose handler)
window._runAlgorithmWithDateRange = function(dateFrom, dateTo) {
  // 凡人話: 大少 adapter.mjs `_brackTestRunDateRangeHandler` 撳「執行」button 後 call 呢個 global helper
  // 拎 dateFrom / dateTo 傳落 runAlgorithm, runAlgorithm 內部已經會 fetch K 線 (filtered) + 重新 render chart + 重新 fetch verdict
  // 對齊 §K-line Cache 永久 rule (8月22日 23:20) — K 線 filtered 喺 K-line Cache layer, frontend 拎 data backend 拎 K 線
  return runAlgorithm(dateFrom, dateTo);
};
...
// 對齊 4.65.0 永久 rule: P 點 + 鮮紫 trigger 一齊 toggle (大少 00:52 trigger「控制這個P點和觸發點的」)
// 大少 9月2日 22:40 trigger (4.66.6) — 改 default true + 加返 localStorage 自動記住 (4.66.5 拎走嘅 spec 拎返)
//   對齊 8月19日 13:03 Config UX 模式永久 rule「自動+手動+自動儲存更新圖表」
...
//   ✅ Mini-table 由 4 欄變 6 欄: 加「觸發點日期」+ 「觸發點股價」2 個 column
//   ✅ 標題上邊加 1 行: 「🔧 Threshold: X.XX% (mode: auto|manual)」
//   ✅ 觸發點: 對齊 4.15.0 永久 rule「之字拎 point 同 trigger 都用 high/low」, Trough trigger 拎嗰日 K 線 high, Peak trigger 拎嗰日 K 線 low
//   ✅ 對齊 4.56.0 精神: 最後 ongoing point 拎 K 線最後 close 做 trigger 價
//   ✅ 凡人話: 大少撳跑 M1 即時喺黑色 console log 底部見到 P1-P10 日子 + 點數 + 觸發點日期 + 觸發點股價
function _formatZigZagLatestPointsForDebug(zigzagPoints, threshold, thresholdMode) {
  ...
  // 大少 4.57.0 trigger 2 — Mini-table header 加 2 個 column: 觸發點日期 + 觸發點股價
  const headerRow = '<tr style="color:#9cdcfe;text-align:left;">' +
    ...
    '<th style="padding-right:12px;">觸發點<br>日期<br><em style="color:#888;font-size:10px;">(trigger 到)</em></th>' +
    '<th style="padding-right:12px;">觸發點<br>股價<br><em style="color:#888;font-size:10px;">(確認價)</em></th>' +
    '</tr>';
...
// 大少 2026-09-14 00:35 — Chart Debug panel 收埋 (大少「我想空出地方來」) 永久 rule:
// 將原本 inline 顯示嘅整個 panel 包入 <details>/<summary>, 預設冇 open → 收埋
// 大少撳「🔧 Chart Debug (點開睇 raw data)」嗰行先睇到內容, 唔阻礙結果框睇圖
// 保留原本所有 9 個 debug field (meta keys / maV2 keys / zigzagEnabled / 紫色 ZigZag series /
// zigzagPoints length / zigzagThreshold / lastSwingHigh / lastSwingLow / K線最後 close) +
// ZigZag P1-P10 mini-table + DevTools console hint, 內容冇刪, 只係由 inline → collapsible
// Browser native <details> 自動處理 open/close 動畫 + a11y (keyboard accessible), 唔需要新 CSS
function renderDebugPanel(chartRefs, verdict, klines) {
  ...
  // details 預設冇 open attribute → 預設收埋, 大少撳先睇
  debugPanel.style.cssText = 'margin-top:14px;';
  const summary = document.createElement('summary');
  summary.style.cssText = 'cursor:pointer;padding:10px 14px;background:#2d2d2d;color:#9cdcfe;border-radius:6px;font-size:13px;font-weight:600;list-style:revert;';
  summary.innerHTML = '🔧 Chart Debug (撳開睇 raw data / 預設收埋)';
  debugPanel.appendChild(summary);
  const debugBody = document.createElement('pre');
  debugBody.style.cssText = 'background:#1e1e1e;color:#d4d4d4;padding:14px;margin-top:6px;border-radius:6px;font-size:12px;line-height:1.6;overflow-x:auto;white-space:pre-wrap;';
  ...
  debugBody.innerHTML = `<strong style="color:#9cdcfe;">🔧 Chart Debug (大少唔使去 console, 直接睇呢度)</strong>
...
if (runFullChainBtn) {
  // 大少 2026-09-16 07:27 — 同 runBtn 一樣 arrow function wrap, 避免 event 漏入 runFullChain (對齊 §Frontend button event listener 永久 rule)
...
function renderTradeJournalSection() {
  ...
  container.innerHTML = `
    <h3 style="margin: 0 0 4px 0; font-size: 16px;">📓 Trade Journal (時光機實戰日誌 — Stage 1+ Followup)</h3>
    <p class="module-purpose" style="margin: 0 0 12px 0;">記低真正落實嘅倉位, 之後用 forward return 同命中率回測驗證</p>
  ...
  // 大少 2026-09-16 07:27 — 同 runBtn 一樣 arrow function wrap, 避免 event 漏入 addTradeJournalEntry (對齊 §Frontend button event listener 永久 rule)
