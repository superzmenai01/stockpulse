...
const ALGO_CACHE_BUST = '5.2.6';  // 大少 2026-09-14 22:47 — M1 Brack Test fix Index 規則 (大少 22:47 trigger「修正 Index 的規則, 不管 Brack Test 怎樣排列, 最上的第一個就是 Index 1, 由大至小排例, 這個 Index 排例要在後台做好」): backend `backend/algorithms/m1_brack_test/algorithm.py` Step 3.6 sort hits by `hits.sort(key=lambda h, reverse=True)` (date_desc) + emit `displayIndex` field 1..N global 落每個 hit + meta emit `displaySortBy="date_desc"` 對齊 spec doc §4.1 Index 規則 (sort 規則永遠寫死喺 spec doc, 對齊 §M1 sub-scenario 永久 rule + §M2 self-check penalty pattern); frontend `renderBrackTestHitTable` Mode A 用 backend displayIndex, Mode B filtered 用 frontend local enumerate 1..M, 不論 mode 第 1 row = Index 1; 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.186 → ?v=2.3.187; 對齊 §Backend hot-reload 永久 rule (8月31日 11:01) 改 backend algo 必 restart + curl verify (大少 22:22 manual verify testing page 撳跑 M1 之後見 error「M1 Brack Test 渲染失敗: renderBrackTestFilterInfo is not a function」): adapter.mjs module 尾 `export { renderBrackTestCard, renderBrackTestFilterInfo }` 加返 renderBrackTestFilterInfo named export (對齊之前 renderBrackTestCard SCOPE fix pattern 2026-09-14 21:35, Root cause: 22:20 trigger 加 renderBrackTestFilterInfo helper 但漏加 named export, testing-page.js runAlgorithm handler dynamic import destructure `{ renderBrackTestFilterInfo }` 拎唔到); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.185 → ?v=2.3.186 (大少 22:20 trigger): (1) 日期由大至小排 (renderBrackTestHitTable sort `(a, b) => String(b.date).localeCompare(String(a.date))`, 大少睇最新嘅 hit 排喺最頂), (2) 日期左邊加 Index column (header 加 `<th>Index</th>` 第一個 column, 每 row 顯示 `hit.index` value, hit dict backend emit 已有 index field 對齊 plan §Verdict shape), (3) tab B 揀 cycle 嗰陣顯示「當前顯示 X 條 / 全部 Y 條 · 🟢 強上升」 (adapter.mjs 新加 renderBrackTestFilterInfo helper, testing-page.js runAlgorithm handler + adapter.mjs _ModeHandler / _CycleHandler 3 個 handler populate `.brack-filter-info` element); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.184 → ?v=2.3.185, 其他要 scroll down 去睇 (大少 22:15 trigger): adapter.mjs BRACK_TEST_PANEL_STYLE 入面加 `.brack-hit-table-scroll { max-height:480px; overflow-y:auto; border:1px solid #ffe0b2; border-radius:4px; }` + `.brack-hit-table thead th { position:sticky; top:0; z-index:1; background:#ffcc80; box-shadow:0 1px 0 #ffa726; }` (對齊表格固定高度 + thead sticky pattern); renderBrackTestCard table HTML 入面 `<tbody>` 上面包 1 個 `<div class="brack-hit-table-scroll">` (大少可以 scroll down 睇其他 rows, 對齊 「只顯示 16 行」 intent, 16 行 ≈ 480px max-height); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.183 → ?v=2.3.184 (大少 22:04 trigger): adapter.mjs renderMAAlignmentV2Result verdict card HTML 入面拎走 renderBrackTestCard() append (line 5533 拎走), 改為 testing-page.js runAlgorithm 之後 conditional render Brack Test card 寫入 #brack-test-panel (chart-section 入面, m6-dashboard-panel 之後, result section 之前, 對齊 §M6 dashboard panel pattern line 1555-1574); adapter.mjs module 尾加 `export { renderBrackTestCard }` 等 testing-page.js dynamic import 拎到; index.html 加 `<div id="brack-test-panel"></div>` 喺 m6-dashboard-panel 之後 (chart-section 入面); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.182 → ?v=2.3.183 (大少 9月14日 21:32 manual verify testing page 撳 Brack Test 跑失敗 error: "ZMEN_SCENARIO_COLOR_MAP is not defined"): adapter.mjs renderBrackTestChartOverlay + renderBrackTestHitTable + renderBrackTestSummary 3 處 reference 拎 ZMEN_SCENARIO_COLOR_MAP (line 1398 in renderMAResult function) 改為 BRACK_TEST_CYCLE_COLOR_MAP (Brack Test 模塊內自定義 11 個 sub_scenario 11 種顏色 const, 對齊原本 ZMEN_SCENARIO_COLOR_MAP line 1398-1411 內容 100%, Root cause: ZMEN_SCENARIO_COLOR_MAP 係 renderMAResult(verdict) function 內部 local const, scope 唔共享; Fix: Brack Test 自己 define 一份同名 map, scope self-contained, future refactor 可以將 ZMEN_SCOPE_COLOR_MAP hoist 做 module-level); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.181 → ?v=2.3.182 (對齊 plan §M1 Brack Test 功能 + 5 隻 stock verify): adapter.mjs 新加 renderBrackTestCard + renderBrackTestChartOverlay + renderBrackTestHitTable + renderBrackTestSummary 4 個 functions (line 5735-5999), 喺 renderMAAlignmentV2Result verdict card HTML 入面 append (line 5533), Brack Test panel 入面有「🎯 跑 Brack Test」button (撳 button 嗰陣 fetch backend /api/algorithms/run?algo=m1_brack_test&symbol=...&data_window_days=1260), 表格 + chart marker 同步 render, 2 個 tab (時間 / 分類) + tab B 入面 dropdown 揀 sub_scenario 過濾 marker + 表格 rows, 11 種 sub_scenario 11 種顏色 reuse ZMEN_SCENARIO_COLOR_MAP (line 1398-1411), 凡人話 card + tab + dropdown + summary 全部 inline style (唔改 generic CSS, 對齊 testing page 永久 rule 唔改 generic CSS); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.180 → ?v=2.3.181 (大少 2026-09-14 00:32 trigger「現在去優化 M1, 在 M1 裡有個『Chart Debug (大少唔使去 console, 直接睇呢度)』你把整個都摺埋放在結果框下邊, 這個一段很少會看, 我想空出地方來」): testing-page.js renderDebugPanel (line 2303-2397) 整個 panel 改用 <details>/<summary> HTML 結構, 預設冇 open → 自動收埋, 大少撳「🔧 Chart Debug (撳開睇 raw data / 預設收埋)」嗰行先睇到內容; 原本 9 個 debug field (meta keys / maV2 keys / zigzagEnabled / 紫色 ZigZag series / zigzagPoints length / zigzagThreshold / lastSwingHigh / lastSwingLow / K線最後 close) + ZigZag P1-P10 mini-table + DevTools console hint 全部保留, 內容冇刪, 只係由 inline 改 collapsible; browser native <details> 自動處理 open/close 動畫 + a11y (keyboard accessible), 唔需要新 CSS; 凡人話: 結果框下邊少咗一大塊黑底 debug 區, 騰出空間畀大少睇圖同其他內容; 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.179 → 2.3.180 (大少 9月12日 07:22 trigger「你再去查下M1.-M6 有沒有同樣的問題,我要的是後台做計算,前台只是顯示」, 對齊 §M7 v2.0.4 Phase 12 永久 rule 沿用): (1) 拎走 frontend algorithms/AS-03-cycle-detection/modules/{ma-alignment,hl-structure,trendline,indicators,volume,volatility}.ts 6 個 file (6 個 file 加埋 191K bytes, 全部 `class XxxModule implements CycleModule<KLine[]>` 計算 class, KLine 入 verdict 出 — frontend 計算違規, 違反「數據處理 Server 內部做」永久 rule 2026-08-23 + 「算法 Backend-only + 模組化」永久 rule 2026-08-22) (2) 拎走 frontend algorithms/AS-03-cycle-detection/std-verdict.ts 348 行 frontend wrapper 純計算 (computeSentiment6D + computeExpectedReturn + computeMaxDrawdownEstimate + toStandardVerdict + runAndStandardize — 全部 frontend 計算, 違反「所有需要計算的都是在後台完成」spirit) (3) 拎走 frontend algorithms/AS-03-cycle-detection/__tests__/{ma-alignment,hl-structure,trendline,indicators,volume,volatility,smoke,standard-verdict}.test.mjs 8 個 frontend test file, 加 backend/tests/test_m1_to_m6.py 對齊 backend M1-M6 emit shape (6 個 pytest, 沿用 §Algorithm Backend-only + 模組化永久 rule) (4) 拎走 frontend algorithms/AS-03-cycle-detection/index.ts 嘅 6 個 module import + CycleDetector class 整個 (frontend umbrella 入口, 完全依賴 6 個 frontend module) + 4 個 orchestrator import/re-export (MultiTFOrchestrator / RegimeChangeAlerter / Aggregator / OrchSynthesizer — smoke.mjs 拎走後冇人 import) + AnalyzeOptions interface + enableFlagsToRecord helper + DEFAULT_ENABLE_FLAGS import + 6 個 module re-export + 1 個 std-verdict.ts re-export (5) 保留 index.ts 嘅 ZmenMAAlignmentModule (大少 2026-08-08 09:13 trigger zmen均算法獨立, 唔屬 7 個 modules) + DecisionEngine re-export (M8 follow-up) + types.ts / config.ts re-export (type defs + config, frontend 拎走後保留 file) (6) Backend emit 拎 evidence curl `/api/algorithms/run?algo={ma_alignment,hl_structure,trendline,indicators,volume_price,volatility}&symbol=HK.00700&data_window_days=1260` 確認 backend emit 完整 verdict meta (M1 33 keys / M2 35 / M3 35 / M4 31 / M5 37 / M6 41, 全部 source of truth ✅) (7) 對齊 §Cache bust self-check 永久 rule 21:24 sync bump ?v=2.3.177 → 2.3.178
