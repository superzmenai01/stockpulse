...
// 大少 2026-09-06 23:17 — M3 fix dataWindowDays backend 對齊: ALGO_CACHE_BUST = '4.69.0' → '4.70.0' (testing-page.js 0 處改 + adapter.mjs 1 處改 + backend/api/algorithms.py 1 處改: backend handler 加 `options["dataWindowDays"] = data_window_days` 將 query param 對齊 algorithm 入面 options.get("dataWindowDays") 拎法 (camelCase) 避免 misleading code default n; adapter.mjs M3 stub line 3466 default 100 改 1260 對齊 testing page 永久 rule 2026-08-14 23:15 (dataWindowDays 永遠用 5 年, 唔再用 100 日默認); 跟 cache bust self-check 永久 rule (21:24) sync bump ?v=2.3.148 → ?v=2.3.149)
const ALGO_CACHE_BUST = '4.70.0';
