...
function initThresholdModeUI() {
  ...
  // 大少 2026-08-21 00:31 — page load 嗰陣同步 lookback 可編輯狀態
  applyLookbackEditable();
...
// 大少 2026-08-21 00:31 trigger — 切換 lookback input 可編輯狀態 (auto 可改, manual 顯示但 disabled)
// 永遠可見 (做大少 manual mode 嘅 reference), 但 manual mode 唔可以改 (改咗都唔影響 manual threshold)
function applyLookbackEditable() {
  const lookbackEl = document.getElementById('zigzag-lookback');
  const resetBtn = document.getElementById('zigzag-lookback-reset-btn');
  if (lookbackEl) lookbackEl.disabled = (getThresholdMode() !== 'auto');
  if (resetBtn) resetBtn.disabled = (getThresholdMode() !== 'auto');
}
...
document.querySelectorAll('input[name="zigzag-mode"]').forEach(r => {
  r.addEventListener('change', async (e) => {
    ...
    const autoDisplay = document.getElementById('zigzag-auto-display');
    const manualDisplay = document.getElementById('zigzag-manual-display');
    applyLookbackEditable();  // 大少 00:31 — lookback input 跟 mode 切可編輯狀態
    if (mode === 'auto') {
...
if (resetAutoBtn) {
  resetAutoBtn.addEventListener('click', async () => {
    ...
    if (autoDisplay) autoDisplay.style.display = '';
    if (manualDisplay) manualDisplay.style.display = 'none';
    applyLookbackEditable();  // 大少 00:31 — lookback 切返可編輯
    if (lastKlines && lastChartRefs) {
      const code = currentOptions.code;
      const period = currentOptions.period || '1d';
      if (code) await applyAutoThreshold(code, period);
