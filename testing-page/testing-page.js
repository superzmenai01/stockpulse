...
const ALGO_CACHE_BUST = '4.98.0';  // 大少 2026-09-12 07:07 — M7 v2.0.4 Phase 12 frontend 拎走 永久 rule (大少 9月12日 07:07 trigger「全面檢查 M7 frontend 有沒有在前台計算的動作,因為所有需要計算的都是在後台完成,前台只是負責顯示信息」): (1) 拎走 frontend algorithms/AS-03-cycle-detection/modules/cycle-synthesizer.ts (276 行, computeMA() 拎 K 線計 MA5/MA20 + synthesizeCycle() 加權綜合 + computeTriggers() 5 個 trigger + computeTransitions() turn-around — frontend 計算違規, 違反「數據處理 Server 內部做」永久 rule 2026-08-23 b2d851ca + 「算法 Backend-only + 模組化」永久 rule 2026-08-22) (2) 拎走 frontend algorithms/AS-03-cycle-detection/modules/synthesizer.ts (515+ 行, 8 stage frontend 重做 — backend algorithm.py 已經 emit 全部 8 stage 結果落 verdict.meta.*, frontend 重做即重覆計算, 違反「所有需要計算的都是在後台完成」spirit) (3) 拎走 frontend algorithms/AS-03-cycle-detection/modules/decision-engine.ts 嘅 `import { synthesizeCycle } from './cycle-synthesizer.ts'` + frontend call (line 1224-1228), 改用 backend emit fallback (line 1230+) 對齊 §M7 v2.0.3 Phase 11 永久 rule 沿用 (9月12日 06:59 trigger) (4) 拎走 frontend algorithms/AS-03-cycle-detection/tests/test-cycle-synth.mjs + __tests__/synthesizer.test.mjs, 加 backend/tests/test_synthesizer.py 對齊 backend 8 stage 計算 (7 個 pytest, 沿用 §Algorithm Backend-only + 模組化永久 rule) (5) Update index.ts 拎走 synthesizer.ts re-export, dead code 拎走 (6) 對齊 §Cache bust self-check 永久 rule 21:24 sync bump ?v=2.3.176 → 2.3.177
